...
/**
 * Immutable value object representing a model row from the {@code models}
 * table.
 *
 * <h3>Why this class exists</h3>
 * <p>
 * The router, provider layer, and chat pipeline all need model metadata —
 * capability tags, speed/quality scores, privacy classification, context window
 * size, and governance attributes — for every routing decision.
 * {@link ModelRecord} provides a typed carrier for this data, replacing ad-hoc
 * {@code Map<String,Object>} access and eliminating repeated DB lookups by
 * allowing the record to be cached and passed by reference.
 *
 * <h3>Design notes</h3>
 * <ul>
 ...
 * API responses.</li>
 * <li>{@link #contextSize} is a boxed {@code Long} so it can be {@code null}
 * when the Ollama manifest does not report a context window, distinguishing
 * "unknown" from "zero".</li>
 ...
 * <ul>
 * <li>v2026.1.0 — initial migration; replaces Node.js plain object model
 * rows</li>
...
public class ModelRecord {
	...
	/**
	 * Lifecycle status: {@code "available"}, {@code "disabled"}, or
	 * {@code "pending"}.
	 */
	...
	/**
	 * Router speed score (0–100); higher values indicate a faster expected
	 * response.
	 */
	public int speedScore;

	/**
	 * Router quality score (0–100); higher values indicate better expected output
	 * quality.
	 */
	...
	/**
	 * ISO-8601 timestamp of the last time this model was seen alive by the model
	 * poller.
	 */
	...
	/**
	 * Resource tier used for quota accounting, e.g. {@code "standard"} or
	 * {@code "gpu"}.
	 */
	public String resourceTier;

	/**
	 * Whether this model requires a GPU to run; used for scheduling and routing
	 * decisions.
	 */
	...
	/**
	 * Cost tier label for external cloud models, e.g. {@code "low"},
