...
/**
 * Connector implementation that synchronises Slack public channel messages into the Olla
 * knowledge base.
 *
 * <h3>Why this class exists</h3>
 * Slack is the primary asynchronous communication tool for many engineering and operations
 * teams. Indexing channel history allows the AI assistant to answer questions about decisions,
 * announcements, and discussions that live in Slack rather than in structured documentation.
 *
 * <h3>Credential format</h3>
 * <pre>{@code
 * {
 *   "token": "xoxb-..."   // Slack bot token with channels:read and channels:history scopes.
 * }
 * }</pre>
 *
 ...
 * <pre>{@code
 * {
 *   "channels": ["general", "engineering"]   // Optional channel filter (not yet used; all
 *                                             // public channels are listed via the API).
 * }
 * }</pre>
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>All Slack Web API responses include an {@code ok} boolean field. This connector checks
 *       {@code ok} on the channel list response and skips individual channel history fetches
 *       that return {@code ok: false} rather than throwing.</li>
 *   <li>Up to 100 public channels are listed, and up to 100 messages are fetched per channel.
 *       Cursor-based pagination is not yet implemented.</li>
 *   <li>Empty messages (e.g. bot events with no text) are silently skipped during content
 *       assembly.</li>
 *   <li>Each channel is stored as a single document whose ID equals the Slack channel ID;
 *       re-syncing replaces the previous snapshot via the deduplication logic in
 *       {@link BaseConnector#ingestDocument}.</li>
 * </ul>
 *
...
public class SlackConnector extends BaseConnector {
     ...
     *
     * <p>The method first lists up to 100 public channels using {@code conversations.list}.
     * For each channel, it fetches the last 100 messages via {@code conversations.history}.
     * Non-empty message texts are aggregated into a single Markdown document per channel
     * and passed to {@link BaseConnector#ingestDocument}. Channels whose history endpoint
     * returns {@code ok: false} are silently skipped.
     *
     ...
     *         channel listing fails or the Slack API returns {@code ok: false}.
     * @since v2026.1.4
     */
    @Override
    ...
    /**
     * Validates that the supplied token can successfully authenticate with the Slack API.
     *
     * <p>The test calls {@code auth.test}, which is the canonical Slack API endpoint for
     * verifying a token. It returns {@code ok: true} on success and identifies the bot or
     ...
     * @return {@code true} if the API call returns {@code ok: true}; {@code false} otherwise.
     * @since v2026.1.4
     */
    @Override
