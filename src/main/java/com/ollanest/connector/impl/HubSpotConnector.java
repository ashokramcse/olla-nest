...
/**
 * Connector implementation that synchronises CRM data from HubSpot into the Olla knowledge base.
 *
 * <h3>Why this class exists</h3>
 * HubSpot is a popular CRM platform used by sales and marketing teams. This connector pulls
 * contacts and deals into the Olla vector store so that AI-assisted queries can surface
 * relevant customer and pipeline data alongside other enterprise content.
 *
 * <h3>Credential format</h3>
 * <pre>{@code
 * {
 *   "apiKey": "..."   // HubSpot private-app access token (Bearer token).
 * }
 * }</pre>
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Contacts are ingested with name, email, job title, and company fields.</li>
 ...
 *       {@code deal-<id>}) so that contacts and deals cannot collide in the store.</li>
 *   <li>Results are capped at 100 per object type; cursor-based pagination is not yet
 *       implemented.</li>
 * </ul>
 *
...
public class HubSpotConnector extends BaseConnector {
     ...
     *
     * <p>Two CRM object types are synced in sequence:
     * <ol>
     *   <li><b>Contacts</b> — up to 100 records with {@code firstname}, {@code lastname},
     *       {@code email}, {@code jobtitle}, and {@code company} properties.</li>
    ...
    public SyncResult sync(Map<String, Object> config, String credentials) {
        ...
        try {
            ...
            JsonNode deals = httpGet(
                    BASE + "/crm/v3/objects/deals?limit=100&properties=dealname,amount,dealstage,closedate",
                    auth);
    ...
    /**
     * Validates that the supplied credentials can successfully call the HubSpot CRM API.
     *
     * <p>The test calls {@code GET /crm/v3/objects/contacts?limit=1}, which is the lightest
     * authenticated request available on the CRM v3 endpoint.
     *
