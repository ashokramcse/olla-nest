...
/**
 * Spring {@code @Component} that pulls Confluence pages into the Olla-Nest knowledge index.
 *
 * <h3>Why this class exists</h3>
 * Confluence is the canonical long-form documentation store for many engineering and product
 * teams. This connector fetches the 50 most-recently-modified pages across all spaces, strips the
 * HTML markup from the page body, and ingests the resulting plain text so the AI layer can answer
 * questions about internal runbooks, architecture decisions, and how-to guides without requiring
 * users to copy-paste content manually.
 *
 ...
 * }
 * </pre>
 * Authentication uses HTTP Basic auth with {@code email:apiToken} encoded in Base64.
 *
 * <h3>Configuration format</h3>
 * No additional {@code config_json} is required. Space and page discovery is automatic.
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Pages are fetched with {@code expand=body.view,space,version} so the HTML body,
 *       space name, and version metadata are all available in a single request.</li>
 *   <li>HTML tags are removed with a simple regex ({@code <[^>]+>}) and whitespace is
 *       collapsed. This is intentionally lightweight — a full HTML parser is not imported
 *       to avoid the extra dependency.</li>
 *   <li>Each page is keyed by its numeric Confluence page ID, making re-syncs idempotent.</li>
 *   <li>Only {@code type=page} content is returned; blog posts and comments are excluded by
 *       the API query.</li>
 * </ul>
 *
...
public class ConfluenceConnector extends BaseConnector {
     ...
     * connector row whose {@code type} column equals {@code "confluence"}.
     *
    ...
    /**
     * Synchronises the 50 most-recently-modified Confluence pages with the Olla-Nest document
     * store.
     *
     * <p>The method performs the following steps:
     * <ol>
     *   <li>Queries {@code /rest/api/content} for the 50 most-recently-modified pages across
     *       all spaces with the page body, space name, and version metadata expanded.</li>
     *   <li>Strips HTML tags from the page body using a regex replacement.</li>
     *   <li>Ingests each page as a Markdown document keyed by the Confluence page ID.</li>
     * </ol>
     *
    ...
    /**
     * Verifies that the supplied credentials can authenticate against the Confluence
     * {@code /rest/api/space?limit=1} endpoint.
     *
