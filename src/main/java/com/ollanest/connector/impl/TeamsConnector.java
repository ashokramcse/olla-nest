...
/**
 * Connector implementation that synchronises Microsoft Teams channel messages into the Olla
 * knowledge base via the Microsoft Graph API.
 *
 * <h3>Why this class exists</h3>
 * Microsoft Teams is the primary communication and collaboration platform for organisations in
 * the Microsoft 365 ecosystem. Indexing Teams channel messages allows the AI assistant to
 * surface team conversations, decisions, and announcements alongside structured documents
 ...
 *                          // Requires Team.ReadBasic.All and ChannelMessage.Read.All scopes.
 * }
 * }</pre>
 *
 * <h3>Design notes</h3>
 ...
 *       ({@code GET /me/joinedTeams}), then all channels in each team, then up to 50 messages
 *       per channel.</li>
 *   <li>Teams message bodies are HTML; the HTML tags are stripped via a simple regex
 *       ({@code replaceAll("<[^>]+>", "")}) to produce plain text.</li>
 *   <li>Per-channel message fetch failures are caught and logged as warnings so that a
 *       single restricted channel does not abort the entire sync.</li>
 *   <li>Each channel is stored as a single document with a composite ID
 *       ({@code <teamId>/<channelId>}) that is stable across renames.</li>
 * </ul>
 *
...
public class TeamsConnector extends BaseConnector {
     ...
     *
     * <p>The method performs a three-level traversal:
     * <ol>
     *   <li>Lists all teams the user has joined via {@code GET /me/joinedTeams}.</li>
     *   <li>For each team, lists all channels via {@code GET /teams/{teamId}/channels}.</li>
     *   <li>For each channel, fetches the last 50 messages via
     *       {@code GET /teams/{teamId}/channels/{channelId}/messages?$top=50}.</li>
     * </ol>
     * Message bodies are stripped of HTML tags before being assembled into a Markdown
     * document and passed to {@link BaseConnector#ingestDocument}. Failures on individual
     * channel fetches are swallowed and logged as warnings.
     *
        ...
        try {
            ...
            for (JsonNode team : teams.path("value")) {
                ...
                for (JsonNode ch : channels.path("value")) {
                    ...
                    try {
                        JsonNode msgs = httpGet(
                                BASE + "/teams/" + teamId + "/channels/" + chId + "/messages?$top=50",
                                auth);
                        StringBuilder sb = new StringBuilder("# " + teamName + " > " + chName + "\n\n");
                        for (JsonNode m : msgs.path("value")) {
                            String body = m.path("body").path("content").asText("")
                                    .replaceAll("<[^>]+>", "").trim();
                            if (!body.isEmpty()) sb.append("• ").append(body).append("\n");
                        }
                        if (ingestDocument(connId, teamId + "/" + chId,
                                teamName + " > " + chName,
