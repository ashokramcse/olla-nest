...
/**
 * Spring {@code @Component} that pulls Discord text-channel messages into the
 * Olla-Nest knowledge index.
 *
 * <h3>Why this class exists</h3> Engineering and product teams increasingly use
 * Discord as an async communication layer. Important decisions, support
 * answers, and design discussions that happen in Discord channels are otherwise
 * invisible to the AI search layer. This connector walks every text channel in
 * a configured guild, fetches the last 100 messages per channel, and ingests
 * them as a single bulleted Markdown document so that channel history becomes
 * searchable.
 *
 ...
 * </pre>
 * 
 * The bot must be a member of the target guild with the
 * {@code Read Message History} permission in each channel to be synced.
 *
 ...
 * <h3>Design notes</h3>
 * <ul>
 * <li>Only channels whose {@code type} equals {@code 0} (GUILD_TEXT) are
 * processed; voice, announcement, and stage channels are skipped.</li>
 * <li>If the bot lacks permission to read a specific channel, that channel is
 * logged at WARN level and skipped; the rest of the sync proceeds
 * normally.</li>
 * <li>Empty messages (e.g., embed-only posts) are filtered out before
 * ingestion.</li>
 * <li>Each channel's entire message batch is stored as one document keyed by
 * the channel's snowflake ID, so re-syncs replace rather than append
 * content.</li>
 * </ul>
 *
...
public class DiscordConnector extends BaseConnector {
	 ...
	 * a persisted connector row whose {@code type} column equals {@code "discord"}.
	 *
	...
	/**
	 * Synchronises the last 100 messages from every text channel in the configured
	 * Discord guild with the Olla-Nest document store.
	 *
	 * <p>
	 * The method performs the following steps:
	 * <ol>
	 * <li>Lists all channels in the guild via
	 * {@code GET /guilds/:guildId/channels}.</li>
	 * <li>Filters to text channels ({@code type == 0}) only.</li>
	 * <li>For each text channel, fetches the last 100 messages.</li>
	 * <li>Concatenates non-empty message content as a bulleted Markdown list and
	 * ingests it as a single document keyed by the channel's snowflake ID.</li>
	 * </ol>
	 *
	 ...
	 *         error message if the guild channel listing fails
	 * @since v2026.1.4
	 */
	@Override
		...
		try {
			...
			for (JsonNode ch : channels) {
				...
				try {
					...
					if (ingestDocument(connId, chId, "#" + chName,
							"https://discord.com/channels/" + guildId + "/" + chId, sb.toString()))
						synced++;
					else
						skipped++;
				} catch (Exception ex) {
