...
/**
 * Spring {@code @Component} that pulls GitLab project content into the Olla-Nest knowledge index.
 *
 * <h3>Why this class exists</h3>
 * Teams that self-host or use GitLab.com keep engineering knowledge in issues and README files
 * that should be searchable via the AI layer. This connector discovers all projects the token has
 * membership access to, ingests open issues, and fetches the {@code README.md} from the default
 * branch so that the AI layer can answer questions about project purpose, setup, and active work
 * without requiring users to manually export content.
 *
 ...
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Projects are discovered via {@code /api/v4/projects?membership=true&per_page=30} so
 *       only projects the token's user is a member of are synced.</li>
 *   <li>Issues are fetched with {@code state=opened} to keep the knowledge index focused on
 *       actionable, current work.</li>
 *   <li>The README is fetched as raw text via the repository files API using
 *       {@code README%2Emd} as the URL-encoded filename and {@code ref=HEAD} to follow the
 *       default branch automatically.</li>
 *   <li>Missing READMEs (404) are silently ignored rather than aborting the project sync.</li>
 *   <li>Issues are keyed by {@code gl-<id>} and READMEs by {@code gl-readme-<projectId>}
 *       to ensure global uniqueness across projects.</li>
 *   <li>The {@code testConnection} method always targets {@code gitlab.com} regardless of the
 *       configured {@code baseUrl}; this is a known limitation and may be refined in a future
 *       version.</li>
 * </ul>
 *
...
public class GitLabConnector extends BaseConnector {
     ...
     * connector row whose {@code type} column equals {@code "gitlab"}.
     *
    ...
    /**
     * Synchronises open issues and README files from all member GitLab projects with the
     ...
     *   <li>For each project, fetches up to 50 open issues and ingests each as a Markdown
     *       document keyed by {@code gl-<issueId>}.</li>
     *   <li>Attempts to fetch the project's {@code README.md} from HEAD and ingests it keyed
     *       by {@code gl-readme-<projectId>}; 404s are silently ignored.</li>
     * </ol>
     *
     * @param  config      connector row from the database; must contain {@code "id"} (connector
     *                     UUID); {@code "config_json"} may optionally specify {@code "baseUrl"}
        ...
        try {
            ...
            for (JsonNode p : projects) {
                ...
                try {
                    ...
                    if (ingestDocument(connId, "gl-readme-" + pid, name + " README", url, rm.asText())) synced++;
                    else skipped++;
    ...
    /**
     * Verifies that the supplied personal access token can authenticate against the public
     * GitLab.com {@code /api/v4/user} endpoint.
     *
