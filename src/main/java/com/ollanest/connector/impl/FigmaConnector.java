...
/**
 * Spring {@code @Component} that pulls Figma file and component metadata into the Olla-Nest
 * knowledge index.
 *
 * <h3>Why this class exists</h3>
 * Design teams document components, patterns, and usage guidelines inside Figma files. This
 * connector walks all projects visible to the authenticated user, fetches file names and component
 * descriptions for each file, and ingests the results so the AI layer can answer questions like
 * "what does the Button component do?" or "which project owns the checkout flow?" without
 * requiring designers to manually export documentation.
 *
 ...
 * No additional {@code config_json} is required. The connector automatically discovers all
 * projects accessible to the authenticated user via {@code GET /me/projects}.
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Component data is fetched via the {@code /files/:key/components} endpoint. If this
 *       call fails (e.g., the file has no components, or the token lacks full file access), the
 *       connector falls back to ingesting just the file name and project name rather than
 *       aborting the sync entirely.</li>
 *   <li>Each file is keyed by its Figma file key ({@code fileKey}), which is stable across
 *       renames, so re-syncs replace rather than duplicate existing documents.</li>
 *   <li>Component descriptions are ingested as a bulleted list under the file heading, making
 *       them directly searchable by component name or purpose.</li>
 * </ul>
 *
...
public class FigmaConnector extends BaseConnector {
     ...
     * connector row whose {@code type} column equals {@code "figma"}.
     *
    ...
    /**
     * Synchronises file names and component descriptions from all Figma projects accessible to
     * the authenticated user with the Olla-Nest document store.
     *
     * <p>The method performs the following steps:
     * <ol>
     *   <li>Lists all projects visible to the token via {@code GET /me/projects}.</li>
     *   <li>For each project, lists all files via {@code GET /projects/:id/files}.</li>
     *   <li>For each file, attempts to fetch component metadata via
     *       {@code GET /files/:key/components} and builds a Markdown document listing each
     *       component name and description.</li>
     *   <li>If component fetching fails, falls back to a minimal document containing just
     *       the file name and project name.</li>
            ...
            for (JsonNode proj : projects.path("projects")) {
                ...
                for (JsonNode file : files.path("files")) {
                    ...
                    try {
                        ...
                        if (ingestDocument(connId, fileKey, fileName, fileUrl, sb.toString())) synced++;
                        else skipped++;
                    } catch (Exception ex) {
                        // Fallback: just name + project
                        if (ingestDocument(connId, fileKey, fileName, fileUrl, "# " + fileName + "\nProject: " + projName)) synced++;
                        else skipped++;
                    }
                }
            }
    ...
    /**
     * Verifies that the supplied personal access token can authenticate against the Figma
     * {@code /me} endpoint.
     *
