...
/**
 * Spring {@code @Component} that pulls GitHub repository content into the Olla-Nest knowledge
 * index.
 *
 * <h3>Why this class exists</h3>
 * Most engineering knowledge lives in Git repositories — README files contain onboarding guides
 * and architecture overviews, and open issues capture bugs, feature requests, and technical debt.
 * This connector fetches both artifact types so the AI layer can answer developer questions such
 * as "how do I set up the auth service?" or "what issues are open around rate limiting?" without
 * requiring manual copy-paste from GitHub.
 *
 ...
 * }
 * </pre>
 * When {@code repo} is absent or blank, the connector lists all repositories in the organisation
 * and syncs each one.
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>README content is fetched from the GitHub Contents API and decoded from Base64 (MIME
 *       variant) before ingestion.</li>
 *   <li>Pull requests are excluded from the issues sync by checking for the presence of the
 *       {@code pull_request} field, which GitHub includes on issues that are also PRs.</li>
 ...
 *   <li>If a repository has no README (e.g., newly created), the 404 from the Contents API
 *       is silently ignored and the sync continues with issues.</li>
 *   <li>Up to 50 repositories and 50 open issues per repository are fetched per sync run;
...
public class GitHubConnector extends BaseConnector {
     ...
     * connector row whose {@code type} column equals {@code "github"}.
     *
    ...
    /**
     * Synchronises README files and open issues from the configured GitHub repository (or all
     * organisation repositories) with the Olla-Nest document store.
     *
     * <p>The method performs the following steps:
     * <ol>
     *   <li>Determines the target repository set: a single repo if {@code repo} is configured,
     *       or all organisation repositories (up to 50) if it is blank.</li>
     *   <li>For each repository, attempts to fetch and ingest the README via the GitHub Contents
     *       API (Base64-decoded).</li>
     *   <li>Fetches open issues sorted by last-updated, skips pull requests, and ingests each
     *       issue as a Markdown document.</li>
     * </ol>
     *
     * @param  config      connector row from the database; must contain {@code "id"} (connector
     *                     UUID) and {@code "config_json"} with at least {@code "owner"}
     ...
     *         if the top-level API call fails
     * @since v2026.1.4
     */
    @Override
    ...
    /**
     * Verifies that the supplied personal access token can authenticate against the GitHub
     * {@code /user} endpoint.
     *
