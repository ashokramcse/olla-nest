...
/**
 * Spring {@code @Component} that pulls Gmail message metadata into the Olla-Nest knowledge index.
 *
 * <h3>Why this class exists</h3>
 * Critical business communications — vendor confirmations, customer escalations, team
 * announcements — arrive via email and are otherwise invisible to the AI search layer. This
 * connector fetches metadata (subject, sender, date) and the Gmail snippet for up to 50 messages
 * from a configurable label so that the AI layer can answer questions like "did we receive a
 * confirmation from Acme Corp?" without requiring users to manually forward emails.
 *
 ...
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Only metadata and the pre-computed snippet are fetched ({@code format=metadata}); full
 *       email bodies are not downloaded, which keeps request volume and storage footprint low
 *       while still providing enough context for most AI search use-cases.</li>
 *   <li>Headers {@code Subject}, {@code From}, and {@code Date} are extracted individually
 *       from the payload headers array using a switch expression.</li>
 *   <li>Each message is keyed by its Gmail message ID, making re-syncs idempotent.</li>
 *   <li>If a specific message cannot be fetched (e.g., it was deleted between the list call
 *       and the detail call), that message is logged at WARN level and skipped; the rest of
...
public class GmailConnector extends BaseConnector {
     ...
     * connector row whose {@code type} column equals {@code "gmail"}.
     *
    ...
    /**
     * Synchronises metadata and snippets for up to 50 Gmail messages from the configured label
     * with the Olla-Nest document store.
     *
     * <p>The method performs the following steps:
     * <ol>
     *   <li>Lists up to 50 message IDs from the configured label (defaulting to
     *       {@code "INBOX"}) via {@code GET /messages?labelIds=:label&maxResults=50}.</li>
     *   <li>For each message, fetches metadata headers ({@code Subject}, {@code From},
     *       {@code Date}) and the pre-computed snippet via
     *       {@code GET /messages/:id?format=metadata}.</li>
     *   <li>Formats the extracted fields as a Markdown document and ingests it keyed by the
     *       Gmail message ID.</li>
     * </ol>
     *
     * @param  config      connector row from the database; must contain {@code "id"} (connector
     *                     UUID); {@code "config_json"} may optionally specify {@code "label"}
        ...
        try {
            ...
            for (JsonNode msg : list.path("messages")) {
                ...
                try {
                    ...
                    if (ingestDocument(connId, msgId, subject.isBlank() ? "Email " + msgId : subject, "https://mail.google.com/mail/u/0/#inbox/" + msgId, text)) synced++;
                    else skipped++;
                } catch (Exception ex) {
    ...
    /**
     * Verifies that the supplied OAuth2 access token can authenticate against the Gmail
     * {@code /profile} endpoint.
     *
