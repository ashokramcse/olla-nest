...
/**
 * Connector implementation that synchronises Notion pages into the Olla knowledge base.
 *
 * <h3>Why this class exists</h3>
 * Notion is widely used as a team wiki and knowledge management platform. This connector
 * indexes all pages accessible to a Notion integration token — including their block-level
 * content — so that the AI assistant can answer questions grounded in the organisation's
 * Notion workspace without requiring users to leave the Olla interface.
 *
 * <h3>Credential format</h3>
 * <pre>{@code
 * {
 *   "token": "secret_..."   // Notion integration token from notion.so/my-integrations.
 * }
 * }</pre>
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>The Notion API requires a custom {@code Notion-Version} header on every request.
 *       This is handled by overriding {@link #httpGet} to inject the header, and by the
 *       private {@link #notionPost} helper for POST requests.</li>
 *   <li>Pages are discovered via the {@code POST /search} endpoint filtered to objects of
 *       type {@code page}. Up to 50 pages are returned per call; cursor-based pagination
 *       is not yet implemented.</li>
 *   <li>For each page, block children are fetched via {@code GET /blocks/{pageId}/children}
 *       (up to 100 blocks). Only blocks with a {@code rich_text} array are rendered; nested
 *       blocks (e.g. children of toggle or column blocks) are not recursed into.</li>
...
public class NotionConnector extends BaseConnector {
     ...
     *
     * <p>The method searches the workspace for all pages accessible to the integration token
     * (up to 50), then fetches the block children for each page (up to 100 blocks). Page
     * content is converted to plain Markdown via {@link #blocksToText} and ingested via
     ...
     *
     * <p>The test sends a {@code POST /search} with {@code page_size=1}, which is the minimal
     * authenticated request and confirms both token validity and integration access.
     *
    ...
    /**
     * Sends an authenticated POST request to a Notion API endpoint, injecting the required
     * {@code Notion-Version} header.
     *
     * @param endpoint the Notion API path segment appended to {@link #BASE}
     *                 (e.g. {@code "search"}).
    ...
    /**
     * Overrides the base {@code httpGet} to inject the {@code Notion-Version} header required
     * by every Notion API request.
     *
     * <p>All GET calls to the Notion API — including block-children fetches — go through this
     * override so that callers do not need to manage the version header themselves.
     *
     * @param url        the full URL to request.
     * @param authHeader the value of the {@code Authorization} header
     *                   (e.g. {@code "Bearer secret_..."}).
     * @return the parsed JSON response body.
     * @throws Exception if the HTTP request fails or the response cannot be parsed.
     * @since v2026.1.4
     */
     ...
     *
     * <p>Notion pages can store their title under different property keys depending on how the
     * database or page was created. This method probes the four most common key names in order:
     * {@code title}, {@code Title}, {@code Name}, {@code name}. Returns {@code "Untitled"} if
     * no matching property is found or an exception occurs.
     *
     ...
     *
     * <p>Only blocks that contain a {@code rich_text} array (e.g. paragraph, heading, to-do,
     * bulleted list item) are rendered; block types without rich text (e.g. divider, image)
     * are silently skipped. Each rendered block is separated by a newline.
     *
     * @param title  the page title, used as the top-level Markdown heading.
     * @param blocks the JSON object returned by {@code GET /blocks/{pageId}/children}.
     * @return a Markdown string starting with {@code "# <title>\n\n"} followed by block text.
     * @since v2026.1.4
     */
