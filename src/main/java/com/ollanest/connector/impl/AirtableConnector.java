...
/**
 * Spring {@code @Component} that pulls Airtable base data into the Olla-Nest knowledge index.
 *
 * <h3>Why this class exists</h3>
 * Airtable is widely used as a lightweight database / project-tracker. This connector walks every
 * table inside a specified base, fetches up to 100 records per table, and ingests each table as a
 * single Markdown document so that the AI search layer can answer questions about structured data
 * stored in Airtable without requiring users to export CSVs manually.
 *
 ...
 * <h3>Design notes</h3>
 * <ul>
 *   <li>The Airtable Metadata API ({@code /meta/bases/:baseId/tables}) is used to discover all
 *       tables without requiring the caller to enumerate them.</li>
 *   <li>Table names are URL-encoded before being used as path segments in the Records API.</li>
 *   <li>Each table is stored as one document keyed by {@code baseId/tableId}, so re-syncs
 *       replace rather than duplicate existing content.</li>
 *   <li>Records are capped at 100 per table ({@code maxRecords=100}) to stay within reasonable
 *       document-size limits for the embedding pipeline.</li>
 * </ul>
 *
...
public class AirtableConnector extends BaseConnector {
     ...
     * connector row whose {@code type} column equals {@code "airtable"}.
     *
    ...
    /**
     * Synchronises all tables in the configured Airtable base with the Olla-Nest document store.
     *
     ...
     *   <li>Serialises each record's fields to Markdown and ingests the result as a single
     *       document keyed by {@code baseId/tableId}.</li>
     * </ol>
     *
     * @param  config      connector row from the database; must contain {@code "id"} (connector
     *                     UUID) and {@code "config_json"} with at least {@code "baseId"}
    ...
    public SyncResult sync(Map<String, Object> config, String credentials) {
        ...
        try {
            ...
            for (JsonNode table : meta.path("tables")) {
                ...
                if (ingestDocument(connId, baseId + "/" + tableId, tableName, "https://airtable.com/" + baseId, sb.toString())) synced++;
                else skipped++;
            }
    ...
    public boolean testConnection(Map<String, Object> config, String credentials) {
        try {
            httpGet("https://api.airtable.com/v0/meta/whoami", "Bearer " + credStr(parseCredentials(credentials), "apiKey"));
            return true;
        } catch (Exception e) {
            return false;
        }
