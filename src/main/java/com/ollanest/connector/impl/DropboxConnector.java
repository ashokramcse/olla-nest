...
/**
 * Spring {@code @Component} that pulls plain-text files from Dropbox into the Olla-Nest knowledge
 * index.
 *
 * <h3>Why this class exists</h3>
 * Teams often keep important reference documents — notes, specs, data exports — in Dropbox
 * folders. This connector recursively walks the user's entire Dropbox root, identifies files with
 * text-compatible extensions, downloads each file's raw content, and ingests it so the AI layer
 * can answer questions about file contents without requiring the user to copy-paste anything.
 *
 ...
 * No additional {@code config_json} is required. The connector always starts from the Dropbox
 * root folder ({@code ""}) with recursive listing enabled.
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Only files with extensions {@code txt}, {@code md}, {@code csv}, or {@code json} are
 *       downloaded; all other file types (binary, images, etc.) are skipped to avoid ingesting
 *       non-textual content into the embedding pipeline.</li>
 *   <li>File content is downloaded via the Dropbox Content API
 *       ({@code content.dropboxapi.com/2/files/download}) using a separate HTTP call from the
 *       metadata listing, as the standard API does not return file content inline.</li>
 *   <li>Each file is keyed by its lower-case Dropbox path ({@code path_lower}), so re-syncs
 *       replace rather than duplicate existing documents.</li>
 *   <li>Download errors for individual files are logged at WARN level and skipped; the rest of
 *       the sync continues normally.</li>
 *   <li>The download request uses a 30-second timeout to handle large files without indefinitely
 *       blocking the sync thread.</li>
 * </ul>
 *
...
public class DropboxConnector extends BaseConnector {
     ...
     * connector row whose {@code type} column equals {@code "dropbox"}.
     *
    ...
    /**
     * Synchronises all plain-text files from the user's Dropbox root (recursively) with the
     ...
     *   <li>Filters to file entries ({@code .tag == "file"}) whose extension is one of
     *       {@code txt}, {@code md}, {@code csv}, or {@code json}.</li>
     *   <li>Downloads each qualifying file's content via the Dropbox Content API.</li>
     *   <li>Ingests each file as a document keyed by its lower-case Dropbox path.</li>
     * </ol>
     *
        ...
        try {
            ...
            for (JsonNode entry : list.path("entries")) {
                ...
                try {
                    ...
                    if (ingestDocument(connId, path, name, "https://www.dropbox.com/home" + path, resp.body())) synced++;
                    else skipped++;
                } catch (Exception ex) {
    ...
    public boolean testConnection(Map<String, Object> config, String credentials) {
        try {
            httpPost("https://api.dropboxapi.com/2/users/get_current_account", "Bearer " + credStr(parseCredentials(credentials), "accessToken"), Map.of());
            return true;
        } catch (Exception e) {
            return false;
        }
