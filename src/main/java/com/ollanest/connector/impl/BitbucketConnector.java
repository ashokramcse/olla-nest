...
/**
 * Spring {@code @Component} that pulls Bitbucket repository issues into the Olla-Nest knowledge
 * index.
 *
 * <h3>Why this class exists</h3>
 * Engineering teams that host code on Bitbucket often use its native issue tracker for bug
 * reports, feature requests, and technical debt. This connector ingests those issues as Markdown
 * documents so that the AI layer can surface relevant issue context during developer Q&amp;A
 ...
 * }
 * </pre>
 * Authentication is performed using HTTP Basic auth ({@code username:appPassword} Base64-encoded).
 *
 ...
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Up to 50 repositories are fetched per sync ({@code pagelen=50}); pagination is not yet
 *       implemented.</li>
 *   <li>Issues are retrieved sorted by most-recently-updated ({@code sort=-updated_on}) so that
 *       fresher context is prioritised on partial syncs.</li>
 *   <li>If the issues endpoint returns an error for a repository (e.g., issue tracker disabled),
 *       that repository is silently skipped rather than aborting the entire sync.</li>
 *   <li>Each issue is keyed by {@code workspace/slug/issue/id} to ensure uniqueness across
 *       repositories.</li>
 * </ul>
 *
...
public class BitbucketConnector extends BaseConnector {
     ...
     * connector row whose {@code type} column equals {@code "bitbucket"}.
     *
    ...
    /**
     * Synchronises issues from all repositories in the configured Bitbucket workspace with the
     * Olla-Nest document store.
     *
     * <p>The method performs the following steps:
     * <ol>
     *   <li>Lists up to 50 repositories in the workspace.</li>
     *   <li>For each repository, retrieves up to 50 issues sorted by last-updated date.</li>
     *   <li>Ingests each issue as a Markdown document keyed by
     *       {@code workspace/slug/issue/id}.</li>
     * </ol>
     *
     * <p>Repositories whose issue tracker is unavailable or restricted are silently skipped;
     * their failure does not affect other repositories in the same run.
     *
     * @param  config      connector row from the database; must contain {@code "id"} (connector
     *                     UUID) and {@code "config_json"} with at least {@code "workspace"}
     * @param  credentials JSON string containing {@code "username"} and {@code "appPassword"}
     * @return {@link SyncResult} with counts of synced and skipped documents, or an error message
     *         if the top-level repository listing fails
     * @since v2026.1.4
     */
    @Override
    public SyncResult sync(Map<String, Object> config, String credentials) {
        ...
        String auth = "Basic " + Base64.getEncoder().encodeToString(
                (credStr(creds, "username") + ":" + credStr(creds, "appPassword")).getBytes());
    ...
    public boolean testConnection(Map<String, Object> config, String credentials) {
        try {
            Map<String, Object> creds = parseCredentials(credentials);
            httpGet(BASE + "/user", "Basic " + Base64.getEncoder().encodeToString(
                    (credStr(creds, "username") + ":" + credStr(creds, "appPassword")).getBytes()));
            return true;
        } catch (Exception e) {
            return false;
