 ...
 * Center articles into the Olla knowledge base.
 *
 * <h3>Why this class exists</h3> Zendesk is a widely used customer support
 * platform. Indexing tickets and Help Center articles allows the AI assistant
 * to answer questions about known issues, product troubleshooting guides, and
...
public class ZendeskConnector extends BaseConnector {
	...
	/**
	 * Synchronises Zendesk tickets and Help Center articles into the Olla knowledge
	 * base.
	 *
	 * <p>
	 * Two content types are synced in sequence:
	 * <ol>
	 * <li><b>Tickets</b> — up to 100 tickets sorted by {@code updated_at DESC}.
	 * Each ticket's subject and description are formatted as a Markdown
	 * document.</li>
	 * <li><b>Help Center articles</b> — up to 100 articles. Article HTML bodies are
	 * stripped of tags before ingestion. Failures on the Help Center fetch (e.g.
	 * feature not enabled) are silently ignored so that ticket sync is not
	 * affected.</li>
	 * </ol>
	 *
	...
	public SyncResult sync(Map<String, Object> config, String credentials) {
		...
		String auth = "Basic " + Base64.getEncoder()
				.encodeToString((credStr(creds, "email") + "/token:" + credStr(creds, "apiToken")).getBytes());
		String connId = (String) config.get("id");
		String base = "https://" + sub + ".zendesk.com";
		int synced = 0, skipped = 0;
		try {
			JsonNode tickets = httpGet(base + "/api/v2/tickets.json?sort_by=updated_at&sort_order=desc&per_page=100",
					auth);
	 ...
	 *
	 * <p>
	 * The test calls {@code GET /api/v2/users/me.json}, which returns the
	 * authenticated agent's user record and is the standard credential-verification
	 * endpoint for Zendesk.
	 *
