 ...
 * <p>Olla Nest supports 20+ external data sources (GitHub, Slack, Notion, Google Drive,
 * Jira, etc.).  Each connector must: authenticate against its provider's API, fetch
 * documents, deduplicate against what was already ingested, and push new or changed
 * content into the RAG store.  {@code BaseConnector} centralises every piece of that
 * pipeline that is identical across all connectors — shared HTTP helpers, SHA-256
 * content-hash deduplication, and RAG ingestion wiring — so that concrete subclasses
 * only need to implement the provider-specific fetching logic.
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Dependencies ({@link JdbcTemplate}, {@link RagService}, {@link CryptoService})
 *       are injected via {@link #setDependencies(JdbcTemplate, RagService, CryptoService)}
 *       rather than a constructor so that {@link ConnectorRegistry} can register subclass
 *       instances without requiring Spring-managed constructor injection.</li>
 *   <li>Content-hash deduplication: before ingesting a document, {@link #ingestDocument}
 *       computes a SHA-256 hash of the content and checks {@code connector_documents}.
 *       If the hash matches the stored hash the document is skipped (no RAG update, no
 *       DB write), keeping sync costs proportional to what actually changed.</li>
 *   <li>Shared HTTP client uses a 15-second connection timeout; per-request response
 *       timeouts are set at 30 seconds in {@link #httpGet} and {@link #httpPost}.</li>
 *   <li>The {@link SyncResult} record aggregates counts for the scheduler's audit log.</li>
 * </ul>
 *
 * <h3>Version history</h3>
 * <ul>
 *   <li><b>v2026.1.4</b> — initial creation; 20-connector framework; SHA-256 dedup;
 *       RAG ingestion wiring; {@link SyncResult} record with factory methods.</li>
 * </ul>
 *
 * @author  Ashok Ram
 * @since   v2026.1.4
...
public abstract class BaseConnector {
    ...
    /** SLF4J logger — uses the concrete subclass name so log lines identify the connector type. */
    protected final Logger log = LoggerFactory.getLogger(getClass());

    /** Shared Jackson mapper for serialising request bodies and deserialising API responses. */
    protected final ObjectMapper mapper = new ObjectMapper();

    /**
     * Shared HTTP client with a 15-second connection timeout.
     * Response timeouts are configured per-request in {@link #httpGet} and {@link #httpPost}.
     */
     ...
     *
     * <p>Called by {@link ConnectorRegistry} immediately after instantiation so that
     * concrete connectors can use the database, RAG service, and crypto service without
     * requiring Spring constructor injection.
     *
     ...
     *
     * <p>The returned string must match the {@code type} column value stored in
     * {@code connector_configs} (e.g. {@code "github"}, {@code "slack"}, {@code "notion"}).
     * Used by {@link ConnectorRegistry} to dispatch sync requests to the correct implementation.
     *
     ...
     *
     * <p>Implementations must fetch all relevant documents from the remote API, call
     * {@link #ingestDocument} for each, and return a {@link SyncResult} summarising
     * how many documents were synced, skipped (unchanged), or deleted.
     * SHA-256 deduplication in {@link #ingestDocument} ensures only changed documents
     * incur a RAG re-ingestion cost.
     *
     * @param  config       the {@code connector_configs} database row (non-secret fields
     *                      such as org name, workspace ID, etc.)
     ...
     *
     * <p>Implementations should make the cheapest possible API call (e.g. fetch the
     * authenticated user profile) to verify that the supplied credentials are valid and
     * the remote endpoint is reachable. Must not modify the RAG store or database.
     *
    ...
    /**
     * Sends an authenticated HTTP GET request and returns the parsed JSON response body.
     *
     * <p>Sets a 30-second response timeout. Throws a {@link RuntimeException} if the
     * HTTP status code is outside the 2xx range, including the URL and status code in
     * the message for easy diagnostics.
     *
     * @param  url         the fully-qualified request URL; must be a valid {@link URI}
    ...
    /**
     * Sends an authenticated HTTP POST request with a JSON body and returns the parsed
     * JSON response.
     *
     * <p>Serialises {@code body} to JSON using the shared {@link ObjectMapper}, sets
     * {@code Content-Type: application/json}, and applies a 30-second response timeout.
     * Throws a {@link RuntimeException} if the HTTP status code is outside the 2xx range.
     *
     * @param  url         the fully-qualified request URL
     * @param  authHeader  the value of the {@code Authorization} header
     * @param  body        the request body as a {@link Map}; serialised to JSON before sending
     * @return             the parsed JSON response body as a {@link JsonNode}
     * @throws RuntimeException  if the response status is not 2xx
     * @throws Exception         on network failure, JSON serialisation, or parse error
     * @since   v2026.1.4
     */
     ...
     *   <li>If found and the stored hash matches, return {@code false} (no change).</li>
     *   <li>If found with a different hash, delete the old RAG document and re-ingest.</li>
     *   <li>If not found, ingest as a new document.</li>
     *   <li>Upsert the {@code connector_documents} row with the new hash and RAG doc ID.</li>
     * </ol>
     *
     * @param  connectorId  the {@code connector_configs.id} value; used as the RAG source tag
     * @param  externalId   the provider-assigned identifier (e.g. GitHub issue number,
     *                      Notion page UUID); must be unique within the connector
     * @param  title        human-readable title for the document (used in RAG retrieval)
     * @param  url          the canonical URL of the document at the provider; may be null
        ...
        if (existing.isEmpty()) {
            ...
            db.update(
                    "UPDATE connector_documents "
                            + "SET title=?, url=?, content_hash=?, rag_doc_id=?, synced_at=? "
                            + "WHERE connector_id=? AND external_id=?",
     ...
     *
     * <p>Used by subclasses to extract individual credential fields (e.g.
     * {@code token}, {@code clientId}, {@code clientSecret}) from the decrypted
     * credentials JSON stored in the database. Returns an empty map on parse failure
     * so callers can safely call {@link #credStr} without a null check.
     *
     * @param  credsJson  the decrypted credentials JSON string; may be null or malformed
     ...
     *
     * <p>Returns an empty string rather than null when the key is absent, simplifying
     * null checks in subclass implementations.
     *
     * @param  creds  the credentials map returned by {@link #parseCredentials}
     * @param  key    the credential field name (e.g. {@code "token"}, {@code "apiKey"})
     ...
     *
     * <p>Used by {@link #ingestDocument} to detect content changes. Returns an empty
     * string on the (extremely unlikely) event that the SHA-256 algorithm is unavailable
     * on the JVM.
     *
     ...
     *
     * <p>Returned by every {@link #sync} implementation and stored in
     ...
     * @param  skipped  number of documents skipped because their content was unchanged
     * @param  deleted  number of documents removed from the RAG store (currently unused;
     *                  reserved for future hard-delete support)
     * @param  error    human-readable error message if the sync failed; {@code null} on
     *                  success
     * @since   v2026.1.4
     */
    public record SyncResult(int synced, int skipped, int deleted, String error) {
        ...
        /**
         * Creates a successful {@link SyncResult} with no errors and zero deleted count.
         *
         ...
         *
         * @return  {@code true} when {@code error} is {@code null}; {@code false} otherwise
         * @since   v2026.1.4
         */
        public boolean isOk() {
            return error == null;
