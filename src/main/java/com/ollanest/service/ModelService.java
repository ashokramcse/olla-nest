 ...
 * <p>The {@code models} table stores raw JDBC rows with JSON-encoded capability arrays,
 *    nullable numeric columns, and provider-specific conventions. Every other service that
 *    needs a strongly-typed {@link ModelRecord} must go through this class, which owns the
 *    mapping logic and the access-control filter. This mirrors the helper functions in
 *    {@code src/models/model.js} ({@code parseModel}) and {@code src/models/user.js}
 *    ({@code allowedModels}) from the prior Node.js implementation.
 *
 * <h3>Design notes</h3>
 * <p>{@link #parseModel(Map)} is deliberately permissive — missing or null columns produce
 *    safe default values so that partial rows (e.g. from model imports) do not crash the
 *    router. The {@code context_size} and {@code max_context_size} columns are treated as
 *    aliases; whichever is non-zero wins.
 *    <p>{@link #allowedModels(com.ollanest.model.User)} applies two independent filters:
 *    the user's access-grant set (from {@link UserService#allowedModelIds}) and the global
 *    {@code allowApiModels} feature flag. API-provider models are excluded when the flag is
 *    {@code false} regardless of individual grants.
 *
...
public class ModelService {
    ...
    /** Service for reading the {@code allowApiModels} feature flag and other settings. */
     ...
     *
     * <p>All field reads are null-safe; missing columns produce sensible defaults
     * ({@code 0} for scores, {@code false} for booleans, empty list for capabilities).
     * The {@code context_size} and {@code max_context_size} columns are coalesced into a
     * single value; a zero result is stored as {@code null} to signal "unknown".
     *
     * @param  row  a raw row map from {@code db.queryForList("SELECT * FROM models …")}
     ...
     *
     * <p>Includes models in every status (available, missing, offline, disabled).
     * Use {@link #allowedModels(com.ollanest.model.User)} to get only the models a
     * specific user may chat with.
     *
     * @return  list of all {@link ModelRecord} instances; empty list when the table is empty
     * @since v2026.1.0
     */
    public List<ModelRecord> getAllModels() {
        List<Map<String, Object>> rows =
            db.queryForList("SELECT * FROM models ORDER BY provider, name");
    ...
    /**
     * Returns the subset of available models that the given user is permitted to use.
     *
     * <p>Applies two independent filters:
     * <ol>
     *   <li>The user's access-grant set, resolved by {@link UserService#allowedModelIds}.</li>
     *   <li>The global {@code allowApiModels} feature flag — models with
     *       {@code provider = "api"} are excluded when the flag is {@code false}.</li>
     * </ol>
     * Only models with {@code status = 'available'} or {@code status = 'configured'} are
     * considered; offline, missing, or disabled models are always excluded.
     *
     * @param  user  the authenticated user whose access grants are evaluated
     * @return       list of models the user may use, in the order returned by the DB query
     * @since v2026.1.0
     */
     ...
     *
     * <p>Returns an empty list for {@code null}, blank, or unparseable input.
     *
    ...
    /**
     * Returns the string value of the first non-null column from a list of candidates,
     * falling back to {@code def} when all are null.
     *
     * @param  row   JDBC result row map
     ...
     *
     * <p>Accepts {@link Boolean}, {@link Number} (non-zero = true), and string
     * ({@code "1"} or {@code "true"}). Returns {@code false} when the column is absent.
     *
     * @param  row  JDBC result row map
     * @param  key  column name
    ...
    /**
     * Returns the integer value of the first non-null column from a list of candidates,
     * falling back to {@code def}.
     *
     * @param  row   JDBC result row map
    ...
    /**
     * Returns the long value of the first non-null column from a list of candidates,
     * falling back to {@code def}.
     *
     * @param  row   JDBC result row map
     * @param  def   default value when all candidate keys are missing
