 ...
 * <p>Olla Nest can have multiple LLM models available simultaneously — small fast local
 *    models, larger reasoning models, specialist coding or vision models, and external API
 *    models. Manually choosing the right model for every message would be tedious and
 *    error-prone. This service automates that decision: it classifies the user's intent into
 *    capability tags, scores every model the user is permitted to access, and returns the
 *    highest-scoring model along with a ranked list of all candidates and a human-readable
 *    explanation. It mirrors the {@code src/services/router.js} logic from the prior Node.js
 *    implementation.
 *
 * <h3>Design notes</h3>
 * <p>The scoring formula is:
 * <pre>
 *   score = capabilityMatch + specialistBonus + speed * speedWeight
 *           + quality * qualityWeight + privacyContribution
 * </pre>
 * Weights are read from the {@code routerWeights} setting (JSON map with keys
 * {@code speed}, {@code quality}, {@code privacy}); defaults are 0.3 / 0.5 / 0.2.
 * <p>Privacy enforcement is hard: when the message contains sensitive content patterns
 * (SSN, credit-card numbers, API keys, medical/PHI terms) or the mode is in the
 * {@code localOnlyModes} list, all non-local models are removed from the candidate set
 * before scoring. External models are not merely downscored — they are eliminated.
 * <p>Admin-configurable regex patterns are loaded from the {@code sensitivePatterns}
 * setting and applied in addition to the built-in patterns.
 *
...
public class RouterService {
    ...
    /** Service for reading persisted settings such as router weights and local-only modes. */
    ...
    public static class RouteResult {
        ...
        /** The model selected for this request; {@code null} if no suitable model was found. */
        public ModelRecord selected;

        /** Capability tags inferred from the message and mode by {@link #classifyRequest}. */
        public List<String> tags;

        /**
         * All scored candidate models in descending score order; each map contains
         * {@code id}, {@code name}, {@code score}, {@code matches}, and {@code breakdown}.
         */
    ...
    /** Matches common medical/PHI terms: diagnosis, prescription, patient record, PHI, HIPAA. */
    private static final Pattern MEDICAL_PAT =
        Pattern.compile("(?i)diagnosis|prescription|patient\\s+record|PHI|HIPAA");
     ...
     *
     * <p>Applies four built-in regex patterns (SSN, credit card, API key, medical/PHI) plus
     * any admin-configured patterns stored in the {@code sensitivePatterns} setting as a JSON
     * array of regex strings. Invalid admin patterns are silently ignored.
     *
     * @param  text  the message text to scan; must not be {@code null}
     * @return       a {@link SensitivityResult} indicating whether sensitive content was found
     *               and listing which categories were matched
     * @since v2026.1.0
     */
    ...
    /**
     * Infers a set of capability tags from a message and an optional UI mode string.
     *
     * <p>Applies keyword pattern matching to the lower-cased message text and maps
     * recognised phrases to capability tags (e.g. {@code "bug"} or {@code "error"} →
     * {@code ["fix", "debugging", "coding"]}). The {@code mode} parameter can override
     * or supplement the text-based classification — for example, mode {@code "build"}
     * always adds {@code ["coding", "build", "project"]} regardless of message content.
     * Returns {@code ["general", "ask"]} when no patterns match.
     *
    ...
    public List<String> classifyRequest(String message, String mode) {
        ...
        if (text.matches(".*?(code|repo|component|api|server|frontend|backend|function|build|generate|create|write a).*")
                || "build".equals(mode))
    ...
    /**
     * Selects the best model for a user request and returns a fully scored candidate list.
     *
     * <p>The routing algorithm:
     * <ol>
     *   <li>Load all models the user is permitted to access via {@link ModelService#allowedModels}.</li>
     *   <li>Classify the request into capability tags using {@link #classifyRequest}.</li>
     *   <li>Detect sensitive content; if found (or if the mode is in {@code localOnlyModes}),
     *       restrict candidates to models with {@code privacy = "local"}.</li>
     *   <li>Score each remaining candidate using capability match, specialist bonus, weighted
     *       speed, weighted quality, and a privacy contribution.</li>
     *   <li>Return the highest-scoring model in {@link RouteResult#selected}.</li>
     * </ol>
     *
     * <p>When the auto-router is disabled (setting {@code routerEnabled = false}) the first
     * model in the allowed list is selected with a score of 0 and the reason explains that
     * the router is off.
     *
     * @param  user     the authenticated user; used to filter the model access list
     * @param  message  the user's message text, used for classification and sensitivity detection
    ...
    public RouteResult routeModel(User user, String message, String mode) {
        ...
        double speedW = weights.containsKey("speed")
            ? ((Number) weights.get("speed")).doubleValue() : 0.3;
        double qualityW = weights.containsKey("quality")
            ? ((Number) weights.get("quality")).doubleValue() : 0.5;
        double privacyW = weights.containsKey("privacy")
            ? ((Number) weights.get("privacy")).doubleValue() : 0.2;
        ...
        for (ModelRecord model : candidates) {
            ...
            double score = capabilityScore + specialistScore
                + model.speedScore * speedW + model.qualityScore * qualityW + privacyScore;
            scored.add(new ScoredModel(model, (int) Math.round(score), matches,
                capabilityScore, model.speedScore * speedW, model.qualityScore * qualityW,
                privacyScore, score));
        }
    ...
    /**
     * Internal holder combining a {@link ModelRecord} with its computed routing scores.
     */
    private static class ScoredModel {
         ...
         */
        ScoredModel(ModelRecord model, int score, List<String> matches,
                int cap, double sp, double qu, double pr, double total) {
            this.model = model;
            this.score = score;
    ...
    /**
     * Safely deserialises a JSON string into a {@link Map}, returning an empty map on failure.
     *
    ...
    /**
     * Safely deserialises a JSON string into a {@link List} of strings, returning an empty
     * list on failure.
     *
