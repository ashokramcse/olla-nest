...
/**
 * Retrieval-Augmented Generation (RAG) pipeline for document ingestion and semantic retrieval.
 *
 * <h3>Why this class exists</h3>
 * <p>Olla Nest allows users to upload documents (PDFs, Markdown, plain text, code) that
 *    are then made available as grounded context for LLM conversations. This service owns
 *    the entire lifecycle: extracting plain text from binary file formats, splitting that
 *    text into overlapping chunks, computing vector embeddings via {@link EmbeddingService},
 *    persisting chunks to the {@code rag_chunks} table, and — at query time — retrieving
 *    the most relevant chunks using cosine similarity (or a keyword-overlap fallback when
 *    embeddings are unavailable).
 *
 * <h3>Design notes</h3>
 * <p>Chunks are paragraph-aware: the text is first split on double newlines and chunks grow
 *    until they exceed {@value #CHUNK_SIZE} characters, at which point the last
 *    {@value #CHUNK_OVERLAP} characters are carried over as overlap to preserve context
 *    across boundaries.
 *    <p>Retrieval uses an in-process cosine similarity scan over all stored chunk embeddings
 *    for the requested scope (global + user-scoped documents). This is intentionally simple
 *    — no external vector database is required — and works well for typical enterprise
 *    document libraries of up to a few thousand chunks. The similarity threshold is
 *    {@value #SIMILARITY_THRESHOLD}; chunks scoring below this value are discarded.
 *    <p>PDF extraction uses Apache PDFBox 3.x via the {@code Loader.loadPDF(byte[])} API.
 *
...
public class RagService {
    ...
    /** Number of characters carried from the end of the previous chunk into the next. */
    private static final int CHUNK_OVERLAP = 200;

    /** Default number of top-scoring chunks returned by {@link #retrieve}. */
    private static final int TOP_K = 5;

    /** Minimum cosine (or keyword) similarity score for a chunk to be included in results. */
    ...
    /**
     * Ingests a document: extracts text, splits into chunks, embeds each chunk, and persists
     * both the document record and its chunks to the database.
     *
     * <p>Text extraction is type-aware: PDF files are processed with Apache PDFBox; all other
     * types (plain text, Markdown, JSON, source code) are read as UTF-8. After extraction the
     * text is chunked with paragraph-aware overlap, each chunk is embedded via
     * {@link EmbeddingService#embed(String)}, and the results are persisted in
     * {@code rag_documents} and {@code rag_chunks}.
     *
     * @param  docId       unique identifier for the document; used as the primary key
     * @param  name        original filename or display name
     * @param  type        MIME type string (e.g. {@code "application/pdf"} or {@code "text/plain"})
     * @param  size        file size in bytes, stored for display purposes
     * @param  inputStream raw byte stream of the document; will be fully consumed and must not
     *                     be {@code null}
     * @param  uploadedBy  user ID or identifier of the uploader
     * @param  scope       visibility scope; {@code "global"} makes the document available to all
     *                     users, any other value restricts it to that specific user or connector
     * @return             a map with keys {@code id}, {@code name}, {@code chunks} (total chunks),
     *                     and {@code embedded} (number of chunks that received an embedding)
    ...
    public Map<String, Object> ingestDocument(String docId, String name, String type, long size,
        ...
        for (int i = 0; i < chunks.size(); i++) {
            ...
            db.update(
                "INSERT INTO rag_chunks (id, document_id, chunk_index, content, embedding_json, created_at) "
                + "VALUES (?, ?, ?, ?, ?, ?)",
     ...
     *
     * <p>First embeds {@code query} using {@link EmbeddingService#embed(String)}. Then
     * loads all chunks belonging to global documents plus any documents scoped to
     * {@code scope} (when {@code scope} is not {@code "global"}). Each chunk is scored:
     * chunks with an embedding use cosine similarity; chunks without one use a
     * keyword-overlap fallback. Results are filtered by {@link #SIMILARITY_THRESHOLD},
     * sorted descending by score, and limited to {@code topK} entries.
     *
     * @param  query  the search query; returns an empty list immediately if blank
     * @param  scope  the user or connector scope to include in addition to global documents;
     *                pass {@code "global"} or {@code null} to retrieve only global documents
     * @param  topK   maximum number of chunks to return; values &le; 0 default to {@value #TOP_K}
    ...
    /**
     * Builds a formatted RAG context string suitable for injection into an LLM system prompt.
     *
     * <p>Calls {@link #retrieve(String, String, int)} with the user's ID as the scope, then
     * formats the top chunks as numbered {@code [Source N]} blocks separated by horizontal
     * rules. Returns {@code null} when no relevant chunks are found so callers can skip
     * prompt injection entirely.
     *
     * @param  query   the user's message or research sub-question used to retrieve context
     * @param  userId  the authenticated user's ID, used to scope retrieval to their documents
     ...
     *
     * <p>Each map in the returned list contains {@code id}, {@code name}, {@code type},
     * {@code size}, {@code chunk_count}, {@code uploaded_by}, {@code scope}, and
     * {@code created_at}.
     *
    ...
    public List<Map<String, Object>> listDocuments() {
        return db.queryForList(
            "SELECT id, name, type, size, chunk_count, uploaded_by, scope, created_at "
     ...
     *
     * <p>Creates a {@code rag_documents} record with {@code type = "text/plain"},
     * {@code uploaded_by = "connector"}, and {@code scope = connectorId}, then splits,
     * embeds, and persists the chunks exactly as {@link #ingestDocument} does.
     *
     ...
     *
     * <p>Chunks are deleted first to respect the foreign-key constraint between
     ...
     *
     * <p>PDF files (detected by {@code .pdf} extension or {@code application/pdf} MIME type)
     * are processed with Apache PDFBox. All other types are decoded as UTF-8 strings.
     *
     ...
     *
     * <p>Text is first split on sequences of two or more newlines (paragraph boundaries).
     * Paragraphs are accumulated into the current chunk until adding the next paragraph
     * would exceed {@link #CHUNK_SIZE} characters, at which point the chunk is finalised
     * and the last {@link #CHUNK_OVERLAP} characters are carried over as the seed of the
     * next chunk. This preserves context across chunk boundaries without duplicating large
     * amounts of text.
     *
     * <p>If the entire algorithm produces no chunks (e.g. the text has no paragraph breaks
     * and fits within the limit) the original text, truncated to {@link #CHUNK_SIZE}, is
     * returned as a single-element list.
     *
     * @param  text  the full document text to chunk; returns an empty list for blank input
    ...
    private List<String> chunkText(String text) {
        ...
        return chunks.isEmpty()
            ? List.of(text.substring(0, Math.min(text.length(), CHUNK_SIZE))) : chunks;
    }
}
