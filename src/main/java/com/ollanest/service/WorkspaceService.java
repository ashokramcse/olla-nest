 ...
 * project files for the system prompt, extracts code artifacts from model output,
 * and writes artifact files to the local filesystem.
 *
 * <h3>Why this class exists</h3>
 * <p>Olla Nest allows users to configure a local project folder as their workspace.
 * This service centralises all file-system interactions so that controllers and the
 * chat pipeline never touch {@code java.io.File} or {@code java.nio.file} directly.
 * It also hosts the logic for extracting fenced code blocks from model output and
 * writing them to the workspace — a feature that mirrors Olla Nest's "save to
 * workspace" capability in the Node.js implementation.
 *
 * <h3>Design notes</h3>
 * <ul>
 ...
 *   <li>The file-tree walker is depth-limited to 4 levels and capped at 50 files so
 *       large monorepo workspaces do not overwhelm the system-prompt token budget.</li>
...
public class WorkspaceService {
    ...
    /** Provides global settings such as {@code workspaceRoot} and {@code localWritesEnabled}. */
    ...
    /** In-process cache of directory file listings, keyed by absolute workspace root path. */
     ...
     *
     * <p>Looks up the user's per-user workspace preference from
     * {@code workspace_prefs}. Falls back to the global {@code workspaceRoot} setting
     * (which itself defaults to {@code <dataDir>/workspace}) if no preference exists.
     * The workspace root is resolved to an absolute path before being returned.
     *
     ...
     * @return       {@code "default"}, {@code "review"}, or {@code "full"};
     *               unknown values are coerced to {@code "default"}
     * @since  v2026.1.0
     */
     ...
     *
     * <p>The walk skips hidden directories (names starting with {@code .}) and
     ...
     * @return                a list of relative file paths; empty if the directory
     ...
     *
     * <p>Some models (e.g. DeepSeek-R1) emit internal reasoning wrapped in
     * {@code <think>} tags before the final response. This method removes those
     * blocks so they are never shown to the user or included in artifact extraction.
     * It also handles the case where the response starts with an unclosed
     * {@code <think>} tag by looking for the first recognisable content marker
     * (code fence, HTML doctype, React import, etc.).
     *
     * @param  content  the raw model output string; may be {@code null}
     * @return          the cleaned output with {@code <think>} blocks removed;
     *                  never {@code null}
     * @since  v2026.1.0
     */
     ...
     * </ol>
     *
     * <p>Artifact filenames are resolved in this priority order:
     * <ol>
     ...
     * @return          a list of extracted {@link Artifact} objects; empty if no
     *                  artifacts could be detected
     * @since  v2026.1.0
     */
    public List<Artifact> extractArtifacts(String content, String message) {
        ...
        if (artifacts.isEmpty() && content.matches(
                "(?si).*?(<!doctype html|<html[\\s>]|import React|from ['\"]react['\"]"
     ...
     *
     * <p>Recognised language tags map directly to their extension. Unknown or empty
     * tags fall back to content-based heuristics (HTML doctype or React indicators).
     * The ultimate fallback is {@code "txt"}.
     *
     ...
     *
     * <p>Applies keyword heuristics for common page types (sign-in, dashboard,
    ...
    /**
     * Converts a string to a URL-safe slug by lowercasing, collapsing non-alphanumeric
     * runs to hyphens, and truncating to 60 characters.
     *
     ...
     *
     * <p>Skips writing if {@code localWritesEnabled} is {@code false} in global
     ...
     * @return            a list of maps describing each file written, with keys
     *                    {@code name}, {@code path}, {@code relativePath}, and
     *                    {@code bytes}; empty if nothing was written
     * @since  v2026.1.4
     */
    public List<Map<String, Object>> writeLocalArtifacts(Map<String, Object> workspace,
            String message, String mode, String content) {
