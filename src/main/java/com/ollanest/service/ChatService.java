 ...
 * system-prompt assembly, message persistence, audit logging, and rate limiting.
 *
 * <h3>Why this class exists</h3>
 * <p>Encapsulates all chat-related business logic that was previously spread across
 * {@code src/services/chat.js} in the Node.js implementation. Centralising these
 * operations here keeps the streaming controllers thin and ensures that message
 * history trimming, system-prompt construction, audit writes, and rate-limit
 * enforcement are applied consistently regardless of which API endpoint initiates
 * a chat turn.
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Context budgeting walks the message history newest-to-oldest and stops when
 *       the estimated token budget (model context window minus reserved tokens for the
 *       system prompt and current user message) is exhausted. This mirrors the
 *       behaviour of the Node.js chat service.</li>
 ...
 *   <li>The rate-limit map is a process-scoped {@link java.util.concurrent.ConcurrentHashMap}
 *       keyed by user ID. It resets the counter whenever more than 60 seconds have
 *       elapsed since the first request in the current window.</li>
 *   <li>The {@link #uid(String)} helper generates URL-safe, base-36 encoded IDs that
 *       sort roughly chronologically, matching the format used by the Node.js ID
 *       generator.</li>
 * </ul>
 *
 ...
 *       added {@link #appendTrace(String, String, String, String, String, List, List, boolean)}</li>
 * </ul>
 *
 * @author  Ashok Ram
 * @since   v2026.1.0
...
public class ChatService {
    ...
    /** Jackson mapper used for serialising/deserialising JSON fields stored in SQLite. */
    ...
    /**
     * Estimates the number of tokens in the given text using a 4-characters-per-token
     * heuristic.
     *
     ...
     *
     * <p>Checks the {@code models} table first (Ollama models), then the
     * {@code api_models} table (external API models). Falls back to 8192 tokens
     * if neither table contains a positive value for the model.
     *
     ...
     *
     * <p>The list is structured as:
     * <ol>
     *   <li>A {@code system} message containing {@code systemPrompt}.</li>
     *   <li>Zero or more historical {@code user}/{@code assistant} messages from
     *       the session, trimmed to fit within the model's context window.</li>
     *   <li>The current {@code user} message, optionally with base64-encoded
     *       {@code images}.</li>
     * </ol>
     *
     * <p>History trimming walks from the most recent message backwards, accumulating
     ...
     * @param  modelName    the model reference used to look up the context window size
     * @param  images       optional list of base64-encoded image strings; may be
     *                      {@code null} or empty
     * @return              an ordered list of message maps with {@code role} and
     *                      {@code content} keys, ready to pass to the model API
     * @since  v2026.1.0
     */
    public List<Map<String, Object>> buildContextMessages(String sessionId, String systemPrompt,
            String userMessage, String modelName, List<String> images) {
     ...
     *
     * @param  mode              the active chat mode (e.g. {@code "ask"}, {@code "build"})
     * @param  route             the router result containing the selected model and reason
     * @param  workspace         the user's resolved workspace map (may be {@code null})
     * @param  projectKnowledge  free-text project knowledge to include in the prompt
     * @return                   the assembled system prompt string
     * @since  v2026.1.0
     */
    public String buildSystemPrompt(String mode, RouterService.RouteResult route,
            Map<String, Object> workspace, String projectKnowledge) {
     ...
     *
     * <p>Constructs the workspace section string (active folder path, permission
     * mode, file list) then delegates final assembly to
     ...
     * @param  ragContext        pre-formatted RAG retrieval snippets; may be {@code null}
     * @return                   the assembled system prompt string
     * @since  v2026.1.4
     */
    public String buildSystemPromptWithRag(String mode, RouterService.RouteResult route,
            Map<String, Object> workspace, String projectKnowledge, String ragContext) {
    ...
    /**
     * Returns the currently active chat session for the user, creating a fresh session
     * (with a welcome assistant message) if none exists.
     *
     * @param  userId  the user's unique identifier
     * @return         a fully assembled chat map including messages, session metadata,
     *                 and boolean flags
     * @since  v2026.1.0
     */
    ...
    /**
     * Archives the user's current active chat session, provided it contains at least
     * one user message. Sessions with only the initial assistant welcome message are
     * not archived.
     *
     ...
     *
     * <p>The returned map includes all session metadata fields ({@code id},
     * {@code userId}, {@code title}, {@code pinned}, {@code archived}, {@code unread},
     * {@code isActive}, {@code createdAt}, {@code updatedAt}) and a {@code messages}
     * list produced by calling {@link #parseMessage(Map)} on each row.
     *
     ...
     *
     * <p>Normalises the {@code live} column (stored as an integer in SQLite) to a
    ...
    public Map<String, Object> parseMessage(Map<String, Object> row) {
        ...
        m.put("live", liveVal == null
            || (liveVal instanceof Number ? ((Number) liveVal).intValue() != 0
     ...
     *
     * <p>Any serialisation or database error is logged at {@code WARN} level and
     * silently discarded so that an audit failure never affects the calling request.
     *
    ...
    public void appendAudit(String actor, String action, String detail, Map<String, Object> extra) {
        try {
            ...
            db.update(
                ...
                + "VALUES (?, ?, ?, ?, ?, ?)",
                uid("audit"), actor, action, detail != null ? detail : "", extraJson,
     ...
     *                          {@code null}
     * @param  tags             capability tags used during routing; may be {@code null}
     ...
     * @since  v2026.1.4
     */
    public void appendTrace(String userId, String sessionId, String message, String mode,
            String selectedModelId, List<String> tags, List<Map<String, Object>> candidates,
            boolean live) {
        try {
            db.update(
                ...
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                uid("trace"), userId, sessionId, message != null ? message : "",
                mode != null ? mode : "", selectedModelId,
     ...
     * sliding window.
     *
     * <p>The counter for a user is reset whenever more than 60 seconds have elapsed
     * since the window was opened. If the counter has reached {@code limitPerMinute},
     * this method returns {@code false} without incrementing. Otherwise the counter
     * is incremented and {@code true} is returned.
     *
     ...
     *
     * <p>The format is {@code <prefix>-<milliseconds-base36>-<random-base36>}, which
     * sorts roughly chronologically and has negligible collision probability for the
     * volumes expected in a single-tenant Olla Nest deployment.
     *
     * @param  prefix  a short alphanumeric prefix (e.g. {@code "chat"}, {@code "msg"})
    ...
    /**
     * Interprets a map entry as a boolean, supporting {@code Boolean}, {@code Number}
     * (non-zero = true), and {@code String} ({@code "1"} or {@code "true"}) values.
     *
     * @param  row  the source map
     * @param  key  the key to look up
