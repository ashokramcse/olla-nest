 ...
 * <p>Olla Nest supports locally-running Ollama as a first-class LLM provider. This service
 *    is the single integration point: it polls Ollama's REST API on a fixed schedule, persists
 *    discovered models into the {@code models} table, infers capabilities and quality/speed
 *    scores from model name heuristics, and exposes a lightweight {@code ping()} check used
 *    by the health endpoint. It deliberately does NOT handle chat inference — that belongs to
 *    {@link ProviderService}.
 *
 * <h3>Design notes</h3>
 * <p>A background {@link Scheduled} task runs every 60 s (5 s initial delay) to reconcile the
 *    local DB with whatever Ollama currently has loaded. If Ollama is unreachable all rows with
 *    {@code provider='ollama'} are flipped to {@code status='offline'} so the router skips them
 *    gracefully. Models no longer reported by Ollama are set to {@code status='missing'}.
 *    <p>The {@code HttpClient} is constructed once in the constructor with a 5-second connect
 *    timeout; individual request timeouts are set per call.
 *    <p>Context-window size is fetched via {@code POST /api/show} and stored in the
 *    {@code context_size} column; failures are silently ignored and the column is left null.
 *
...
public class OllamaService {
    ...
    /** Jackson mapper used for serialising request bodies and parsing Ollama JSON responses. */
     ...
     *
     * <p>Reads the {@code ollamaUrl} setting from the database, falling back to
     * the value injected from {@code application.properties} if no setting is stored.
     *
     ...
     *
     * <p>Ensures that callers can safely append path segments with a leading {@code /}
     * without creating double-slash URLs.  Returns {@link #defaultOllamaUrl} when
     * {@code url} is {@code null}.
     *
     * @param url  raw URL string, may be {@code null} or end with one or more {@code /}
     ...
     *
     * <p>The initial delay of 5 seconds gives the application time to finish startup
     * before the first network call is made. Errors are caught and logged at WARN level
     * so that a temporarily unreachable Ollama instance does not crash the scheduler.
     *
     * @since   v2026.1.0
     */
     ...
     *
     * <p>For each model reported by Ollama this method upserts a row in the {@code models}
     * table, setting {@code provider='ollama'}, inferred capabilities, speed/quality scores,
     * and the model's context window size (queried from {@code /api/show}).  Models that were
     * previously in the DB but are no longer installed are set to {@code status='missing'}.
     * If Ollama is unreachable all Ollama rows are set to {@code status='offline'} and the
     * method returns {@code ok:false}.
     *
     * <p>After upserting, {@link #ensureDefaultAccess()} is called to bootstrap access grants
     * when none exist yet.
     *
     * @return a map with keys {@code ok} (boolean), {@code models} (list of raw Ollama model maps),
     *         and optionally {@code error} (string) on failure
     * @since   v2026.1.0
     */
    public Map<String, Object> syncOllamaModels() {
        ...
        if (!seenIds.isEmpty()) {
            ...
            db.update("UPDATE models SET status = 'missing' WHERE provider = 'ollama' AND id NOT IN ("
     ...
     *
     * <p>On a fresh installation the {@code access_grants} table is empty. This method
     * inserts a "group-all can use" grant for every Ollama model so that users can start
     * chatting immediately without manual admin setup.  Once any grant exists the method
    ...
    private void ensureDefaultAccess() {
        ...
        for (String modelId : ollamaModels) {
            db.update(
                "INSERT INTO access_grants (id, subject_type, subject_id, model_id, can_use) "
                + "VALUES (?, 'group', 'group-all', ?, 1)",
                uid("grant"), modelId);
        }
    }

    /**
     * Calls {@code GET /api/tags} on the given Ollama base URL and returns the list of models.
     *
     * <p>Each map in the returned list contains raw fields from the Ollama API response such as
     * {@code name}, {@code size}, {@code digest}, and {@code details}.  Throws if the HTTP
     * status code is not 200 or if the request times out (3-second timeout).
     *
     * @param  baseUrl  Ollama base URL without trailing slash, e.g. {@code http://localhost:11434}
     * @return          list of raw model maps; empty list if the {@code models} array is absent
     * @throws Exception  if the HTTP request fails, times out, or Ollama returns a non-200 status
     * @since   v2026.1.0
     */
     ...
     *
     * <p>Inspects the {@code modelinfo} object for any field ending in
     * {@code .context_length}; falls back to parsing a {@code num_ctx} line from
     * the {@code parameters} string if the field is absent.  Returns {@code null}
     * on any error or when the value cannot be determined.
     *
        ...
        try {
            ...
            if (params != null) {
                java.util.regex.Matcher m =
                    java.util.regex.Pattern.compile("num_ctx\\s+(\\d+)").matcher(params.asText(""));
     ...
     *
     * <p>Uses simple regex matching on the lowercased model name to assign tags such as
     * {@code coding}, {@code reasoning}, {@code vision}, {@code medical}, etc.  Every model
     * receives at least {@code general} and {@code ask}.  These tags are stored as a JSON
     * array in the {@code capabilities} column and drive the auto-router's scoring.
     *
     * @param  modelName  Ollama model reference string (e.g. {@code qwen2.5-coder:7b})
    ...
    /**
     * Computes heuristic speed and quality scores for a model based on its name and disk size.
     *
     * <p>Speed is derived from the model's size on disk: smaller models score higher because
     * they typically run faster on consumer hardware. Quality starts from a size-based baseline
     * and is bumped upward for models whose names suggest strong reasoning or specialist ability.
     * Both scores are clamped to the range [10, 100].
     *
     * @param  modelName  Ollama model reference (used for name-pattern quality bonus)
     * @param  sizeBytes  model file size in bytes as reported by Ollama; pass {@code 0} if unknown
     * @return            two-element array {@code [speedScore, qualityScore]}, each in [10, 100]
     * @since   v2026.1.0
     */
     ...
     *
     * <p>Sends a lightweight {@code GET /api/tags} request with a 2-second timeout.
     * Any exception (connection refused, timeout, etc.) is caught and returns {@code false}.
     * This method is safe to call from health-check endpoints.
     *
     ...
     *
     * <p>Combines the current epoch millisecond and a random component, both encoded in
     * base-36, to produce a compact, monotonically-increasing unique string.
     *
