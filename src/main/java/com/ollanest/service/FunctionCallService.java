...
/**
 * Defines AI-callable tool specifications and executes tool calls dispatched by the
 * Ollama function-calling protocol.
 *
 * <h3>Why this class exists</h3>
 * <p>Ollama models that support function/tool calling return structured
 * {@code tool_calls} objects in their response instead of (or in addition to) plain
 * text. This service is the single location where those tool definitions are declared
 * and where the corresponding business logic is executed, keeping the streaming
 * controllers free of tool-specific code.
 *
 * <h3>Design notes</h3>
 * <ul>
 ...
 *   <li>The {@code calculate} tool passes the expression through a whitelist regex
 *       before evaluation to prevent injection attacks via the JavaScript engine.</li>
 *   <li>The JavaScript engine ({@code ScriptEngineManager}) is used for expression
 *       evaluation because Java 21 ships GraalJS via the JDK; if no engine is
 *       available a graceful fallback message is returned.</li>
 ...
 *   <li>v2026.1.4 — added {@code search_knowledge_base} and {@code get_system_info}
 *       tools; added double-serialised argument parsing in
...
public class FunctionCallService {
        ...
        tools.add(buildTool("calculate",
            ...
            Map.of("type", "object",
                "properties", Map.of("expression", Map.of("type", "string",
                    "description", "A valid mathematical expression, e.g. '(15 * 8) / 100 + 42'")),
     ...
     *
     * <p>Dispatches to the appropriate private implementation method based on
     * {@code toolName}. Returns a JSON string in all cases — either a success payload
     * or an {@code {"error": "..."}} object.
     *
    ...
    public String executeTool(String toolName, Map<String, Object> args, String userId) {
        return switch (toolName) {
            ...
            case "search_knowledge_base" ->
                executeSearchKnowledgeBase((String) args.getOrDefault("query", ""), userId);
    ...
    /**
     * Parses the {@code tool_calls} array from an Ollama model response message node.
     *
     * <p>Each entry in the returned list is a map with keys {@code name} (the tool
     * function name) and {@code args} (a map of argument name to string value).
     * Handles both directly-object argument nodes and text-encoded (doubly-serialised)
     * argument nodes produced by some Ollama model versions.
     *
     * @param  message  the {@code message} JSON node from the Ollama response; may be
     *                  {@code null}
     * @return          a list of parsed tool-call maps, empty if no tool calls are
     *                  present or {@code message} is {@code null}
     * @since  v2026.1.4
     */
    ...
    private String executeGetDatetime() {
        ...
        catch (Exception e) { return "{\"datetime\":\"" + now + "\"}"; }
    }

    /**
     * Evaluates a mathematical expression string and returns the numeric result.
     *
     * <p>The expression is sanitised by stripping all characters except digits,
     * basic arithmetic operators, parentheses, the percent sign, and spaces before
     * being passed to the JavaScript engine. Expressions containing only unsafe
     * characters after sanitisation are rejected.
     *
     * @param  expression  the mathematical expression to evaluate
     * @return             a JSON string with keys {@code expression} and {@code result},
    ...
    /**
     * Searches the user's RAG knowledge base for documents relevant to the query and
     * returns the top three results.
     *
    ...
    private String executeGetSystemInfo() {
        ...
        try { return mapper.writeValueAsString(info); }
        catch (Exception e) { return "{\"product\":\"Olla Nest\"}"; }
    }
     ...
     * @return             a map with {@code type="function"} and a nested
     *                     {@code function} map
     */
