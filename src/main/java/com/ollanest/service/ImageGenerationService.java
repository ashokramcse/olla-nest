...
/**
 * AI image generation service supporting DALL-E 3 and self-hosted Stable Diffusion.
 *
 * <h3>Why this class exists</h3>
 * <p>Olla Nest users can generate images directly from the chat composer by clicking the
 * image generation button. {@code ImageGenerationService} is the single integration point
 * for image-generation APIs — it reads provider configuration from the settings table and
 * dispatches to the appropriate backend, returning a uniform {@link ImageResult} record
 * regardless of which provider produced the image. Controllers and chat flows never touch
 ...
 *   <li><b>Stable Diffusion</b> — Automatic1111 WebUI API at a self-hosted base URL
 *       configured via {@code sdBaseUrl} (default: {@code http://localhost:7860}).
 *       Returns a base64-encoded PNG string.</li>
 * </ul>
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>Provider selection is driven by the {@code imageProvider} setting, or overridden
 *       per-request by the caller (e.g., {@code ImageController}).</li>
 *   <li>DALL-E calls have a 60-second timeout; Stable Diffusion has 120 seconds because
 *       local GPU inference can be slower.</li>
 *   <li>The service does not store generated images — it passes the URL or base64 back
 *       to the caller for inline display in the chat UI.</li>
 * </ul>
 *
 * <h3>Version history</h3>
 * <ul>
 *   <li><b>v2026.1.4</b> — initial creation; DALL-E 3 and Stable Diffusion (Automatic1111)
 *       backends; {@link ImageResult} record; configurable model, size, and SD parameters.</li>
 * </ul>
 *
 * @author  Ashok Ram
 * @since   v2026.1.4
...
public class ImageGenerationService {
    ...
    /**
     * HTTP client shared across all image-generation calls; 15-second connection timeout.
     * Note: response timeouts are set per-request because DALL-E and SD have very different
     * expected latencies.
     */
     ...
     *
     * @param  dbService  the database service for reading provider settings and API keys
     ...
     *
     * @param  url       a time-limited CDN URL to the generated image (DALL-E), or {@code null}
     *                   if the result is base64-encoded
     * @param  base64    base64-encoded PNG data (Stable Diffusion), or {@code null} if a URL
     *                   was returned instead
     * @param  provider  the provider that generated the image ({@code "dalle"} or
     *                   {@code "stable-diffusion"})
     * @param  model     the specific model used (e.g. {@code "dall-e-3"} or {@code "sd"})
     * @since   v2026.1.4
     */
    ...
    /**
     * Generates an image for the given prompt using the configured (or requested) provider.
     *
     * <p>If {@code requestedProvider} is non-null and non-blank it overrides the system
     * setting. The strings {@code "stable-diffusion"} and {@code "sd"} both select the
     * Stable Diffusion backend; anything else (including {@code "dalle"}) selects DALL-E 3.
     *
     * @param  prompt             the natural-language description of the image to generate;
     *                            must not be null or blank
     * @param  requestedProvider  optional provider override ({@code "dalle"}, {@code "sd"},
     *                            {@code "stable-diffusion"}); use {@code null} to read from settings
     * @return                    an {@link ImageResult} with either {@code url} or {@code base64}
     *                            populated depending on the provider
     * @throws Exception  if the provider API returns an error, the API key is missing,
     ...
     *
     * <p>Reads {@code imageModel} (default: {@code "dall-e-3"}) and {@code imageSize}
    ...
    private ImageResult generateDalle(String prompt) throws Exception {
        ...
        if (root.has("error")) {
            throw new RuntimeException(
                    "DALL-E error: " + root.path("error").path("message").asText());
        }
    ...
    /**
     * Generates an image via a self-hosted Stable Diffusion Automatic1111 WebUI API.
     *
     * <p>POSTs to {@code {sdBaseUrl}/sdapi/v1/txt2img} with a fixed set of generation
     * parameters (20 steps, 512×512, Euler a sampler, negative prompt filtering NSFW/blurry
     * content). The response contains base64-encoded PNG images in the {@code images} array.
     *
     * @param  prompt  the image description
    ...
    private ImageResult generateSD(String prompt) throws Exception {
        ...
        HttpResponse<String> resp = http.send(req, HttpResponse.BodyHandlers.ofString());
        JsonNode root = mapper.readTree(resp.body());
        if (!root.has("images") || !root.path("images").isArray()
                || root.path("images").isEmpty()) {
     ...
     *
     * <p>Currently reads the {@code openaiApiKey} setting directly. Future versions
