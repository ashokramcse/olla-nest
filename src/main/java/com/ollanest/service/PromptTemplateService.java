...
/**
 * Assembles the system prompt for each chat turn by combining an identity preamble,
 * routing metadata, workspace context, project knowledge, RAG snippets, and
 * mode-specific behaviour instructions.
 *
 * <h3>Why this class exists</h3>
 * <p>The system prompt is the primary mechanism by which Olla Nest shapes model
 ...
 * shape, and makes it easy to iterate on wording without touching controller logic.
 *
 * <h3>Design notes</h3>
 * <ul>
 ...
 *   <li>Mode instructions are stored in a static {@link LinkedHashMap} so iteration
 *       order matches insertion order, which is useful when listing modes in the UI.</li>
 *   <li>Unknown mode values fall back to the {@code "ask"} instruction set so the
 *       prompt is always well-formed even if a client sends an unrecognised mode.</li>
 * </ul>
 *
 ...
 *   <li>v2026.1.4 — added {@code ragSection} placeholder and the {@code learn} mode
 *       instruction</li>
 * </ul>
 *
 * @author  Ashok Ram
...
public class PromptTemplateService {
    ...
    /**
     * Per-mode behaviour instructions injected into the system prompt.
     * Insertion order is preserved for UI display purposes.
     */
    ...
    static {
        ...
        MODE_INSTRUCTIONS.put("review",
            "Review the request for issues, risks, improvements, and missing pieces. "
    ...
    public String buildSystemPrompt(String mode, String modelName, String routeReason,
        ...
        vars.put("workspaceSection",
            workspaceInfo != null && !workspaceInfo.isBlank() ? workspaceInfo : "");
        vars.put("projectKnowledgeSection",
            projectKnowledge != null && !projectKnowledge.isBlank()
                ? "Project Knowledge:\n" + projectKnowledge.trim() : "");
        vars.put("ragSection",
            ragContext != null && !ragContext.isBlank() ? ragContext : "");
     ...
     *
     * <p>Useful for admin endpoints that expose the list of available modes and their
     * instructions to the frontend.
     *
     * @return  an unmodifiable {@link Map} from mode name to instruction text,
     *          in insertion order
     * @since  v2026.1.0
     */
