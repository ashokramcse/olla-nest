...
/**
 * Provider call layer — resolves the active provider and executes both blocking and
 * streaming LLM inference requests.
 *
 * <h3>Why this class exists</h3>
 * <p>Olla Nest supports multiple LLM backends (Ollama, Anthropic, OpenAI, Groq, and
 *    arbitrary OpenAI-compatible endpoints). Rather than scattering HTTP logic throughout
 *    controllers and pipeline services, all provider-specific wiring lives here. Callers
 *    receive a uniform {@link ProviderResult} or stream their tokens via a {@link Consumer}
 *    without needing to know which wire protocol is in use. This mirrors the JavaScript
 *    {@code src/services/providers.js} module from the prior Node.js implementation.
 *
 * <h3>Design notes</h3>
 * <p>A single shared {@link HttpClient} is constructed in the constructor with a 10-second
 *    connect timeout; per-request timeouts are set individually (streaming calls use a
 *    5-minute timeout to accommodate large model outputs).
 *    <p>{@link FunctionCallService} is injected via a setter rather than the constructor to
 *    break the circular dependency that would otherwise occur
 *    ({@code ProviderService} → {@code FunctionCallService} → {@code ProviderService}).
 *    <p>API keys are stored encrypted; {@link CryptoService} handles decryption transparently
 *    so that keys are never logged or held in plain text longer than necessary.
 *
...
public class ProviderService {
    ...
    /**
     * Set via {@link #setFunctionCallService(FunctionCallService)} after construction
     ...
     *
     * <p>Tool-call responses (when the model returns structured function invocations instead
     * of plain text) are surfaced via the {@link #toolCalls} field; it is {@code null} for
     * ordinary text completions.
     */
    public static class ProviderResult {
        ...
        /**
         * Non-{@code null} when the model responded with structured tool calls instead of
         * (or in addition to) plain text; each map contains at least {@code name} and
         * {@code arguments} keys.
         */
     ...
     *
     * <p>For the built-in {@code ollama} provider a synthetic map is returned using the
     * current Ollama URL from settings. For all other providers the {@code api_providers}
     * table is queried; a {@link RuntimeException} is thrown if the provider is not found
     * or is disabled.
     *
     * @param  route  the routing decision produced by {@link RouterService#routeModel}
    ...
    public Map<String, Object> resolveProvider(RouterService.RouteResult route) {
        ...
        if (!isEnabled) {
            throw new RuntimeException(
                "Provider '" + p.get("name") + "' is disabled. Enable it in Admin → Providers.");
        }
        return p;
    }
     ...
     * {@link #callProvider(Map, String, List, int, List)} with {@code tools = null}.
     *
     * @param  provider   provider configuration map from {@link #resolveProvider}
     * @param  modelId    the model reference to pass to the provider (e.g. {@code llama3.2:3b})
     * @param  messages   ordered list of chat messages; each map must have {@code role}
     *                    and {@code content} keys
     * @param  timeoutMs  per-request timeout in milliseconds; values &le; 0 default to
     *                    300 000 ms (5 minutes)
     * @return            the provider's completion result
     * @throws Exception  if the HTTP request fails, times out, or the provider returns a
     *                    non-200 status
     * @since v2026.1.0
     */
    public ProviderResult callProvider(Map<String, Object> provider, String modelId,
            List<Map<String, Object>> messages, int timeoutMs) throws Exception {
        return callProvider(provider, modelId, messages, timeoutMs, null);
    }

    /**
     * Executes a blocking (non-streaming) LLM request, optionally with tool definitions.
     *
     * <p>Dispatches to the appropriate provider implementation based on the {@code type}
     * field of {@code provider}: {@code "ollama"}, {@code "anthropic"}, or any
     * OpenAI-compatible type ({@code "openai"}, {@code "groq"}, or custom). Responses
     * are parsed and normalised into a {@link ProviderResult}.
     *
     * <p>For Ollama calls the model output is cleaned via
     * {@link WorkspaceService#cleanModelOutput(String)} to remove artefacts. Tool-call
     * responses are parsed by {@link FunctionCallService#parseToolCalls} and stored in
     * {@link ProviderResult#toolCalls}.
     *
     * @param  provider   provider configuration map from {@link #resolveProvider}
     * @param  modelId    the model reference to pass to the provider
     * @param  messages   ordered list of chat messages
     * @param  timeoutMs  per-request timeout in milliseconds; &le; 0 defaults to 5 minutes
     * @param  tools      optional list of tool definition maps; {@code null} or empty to
     ...
     */
    public ProviderResult callProvider(Map<String, Object> provider, String modelId,
            List<Map<String, Object>> messages, int timeoutMs, List<Map<String, Object>> tools)
            throws Exception {
        ...
        if ("ollama".equals(type)) {
            ...
            ProviderResult result = new ProviderResult(
                WorkspaceService.cleanModelOutput(content), tokens,
            ...
            String base = ollamaService.cleanBaseUrl(
                (String) provider.getOrDefault("base_url", "https://api.anthropic.com"));
            ...
            JsonNode data = mapper.readTree(resp.body());
            String content = data.has("content") && data.get("content").isArray()
                && data.get("content").size() > 0
            ...
            JsonNode data = mapper.readTree(resp.body());
            String content = "";
            if (data.has("choices") && data.get("choices").isArray()
                    && data.get("choices").size() > 0) {
    ...
    /**
     * Executes a streaming LLM request and delivers tokens incrementally via a callback.
     *
     * <p>Dispatches to the appropriate provider protocol: Ollama NDJSON,
     * Anthropic Server-Sent Events, or OpenAI-compatible SSE. Each incoming token is
     * passed to {@code onToken}; when the stream ends (or on error) {@code onDone} is
     * called with the total token count (which may be {@code 0} for providers that do
     * not report token usage in the stream).
     *
     * @param  provider   provider configuration map from {@link #resolveProvider}
     ...
     *                    {@code null}
     * @param  onDone     callback invoked once when the stream is exhausted; receives the
     *                    total token count; must not be {@code null}
     * @throws Exception  if the HTTP request fails or the provider returns a non-200 status
     * @since v2026.1.0
     */
    public void callProviderStream(Map<String, Object> provider, String modelId,
            List<Map<String, Object>> messages, Consumer<String> onToken,
            Consumer<Integer> onDone) throws Exception {
        String type = (String) provider.get("type");
        String apiKey = cryptoService.decryptKey((String) provider.get("api_key_enc"));
        int[] totalTokens = {0};
     ...
     *
     * <p>Returns the configured {@code base_url} if present and non-blank. Otherwise falls
     * back to the standard Groq endpoint ({@code https://api.groq.com/openai}) for
     * {@code type = "groq"} or the standard OpenAI endpoint
     * ({@code https://api.openai.com/v1}) for all other types.
     *
     ...
     *
     * <p>When an API model's {@code is_approved} flag is {@code true} it is upserted into
     * {@code models} so that the router can select it like any Ollama model. When
     * {@code is_approved} is {@code false} the corresponding row is deleted. The composite
     * model ID is formed as {@code <providerId>:<modelId>}.
     *
     * @param  provider  provider configuration map (must contain {@code id} and {@code name})
    ...
    public void mirrorApiModelToModels(Map<String, Object> provider, Map<String, Object> apiModel) {
        ...
        boolean approved = isApproved != null && (isApproved instanceof Boolean
            ? (Boolean) isApproved : ((Number) isApproved).intValue() != 0);
