 ...
 * <p>Flyway owns DDL (table creation) but cannot know runtime defaults such as the
 *    admin e-mail, the Ollama base URL, or the initial role and permission catalogue.
 *    This service fills that gap: immediately after Spring finishes wiring beans it runs
 *    {@link #seedDatabase()}, which inserts default rows into every reference table that
 *    is still empty. The checks are idempotent — if a table already has rows the seed
 *    is skipped — so restarts and rolling upgrades are safe.
 *
 * <h3>Design notes</h3>
 * <p>All DB access goes through {@link JdbcTemplate} using {@code INSERT OR REPLACE} or
 *    {@code INSERT OR IGNORE} SQLite syntax so that no external transaction management is
 *    needed. Passwords are hashed with BCrypt (cost factor 12) at seed time; a first-boot
 *    sentinel value causes a cryptographically random admin password to be generated and
 *    printed to the log as a one-time bootstrap credential.
 *    <p>The {@link #getSetting(String, String)} / {@link #setSetting(String, String)} pair
 *    acts as a lightweight key-value store layered on top of the {@code settings} table,
 *    consumed by many other services.
 *
...
public class DatabaseService {
     ...
     *
     * @param db        Spring JDBC template bound to the application's SQLite data source
    ...
    /**
     * Entry point called by Spring immediately after dependency injection is complete.
     *
     * <p>Delegates to each individual seed method in dependency order:
     * settings → departments → groups → permissions → roles → users.
     * Any exception is caught and logged at ERROR level; startup continues regardless so
     * that a seed failure does not prevent the application from running.
     *
     * @since v2026.1.0
     */
    @PostConstruct
     ...
     *
     * <p>Used by each seed method to determine whether the table is empty and therefore
     * requires seeding. Returns {@code 0} when the count query returns {@code null}.
     *
     ...
     *
     * <p>Returns {@code fallback} when the key does not exist, when the stored value is
     * {@code null}, or when any SQL exception occurs (e.g. the table does not yet exist
     ...
     *
     * <p>Interprets the stored string value as a boolean: {@code "true"} returns
    ...
    /**
     * Persists a string setting into the {@code settings} table, replacing any existing value.
     *
     * <p>Uses SQLite {@code INSERT OR REPLACE} so this method is safe to call whether the
     * key already exists or is new.
     *
     ...
     *
     * <p>Inserts the Ollama base URL (read from the {@code OLLAMA_URL} environment variable
     * with a fallback of {@code http://localhost:11434}), the default workspace root, and a
     * set of feature-flag defaults. Skipped entirely when the table already contains rows.
     *
     * @since v2026.1.0
     */
     ...
     *
     * <p>Inserts General, Product, and Support departments. Skipped when the table already
     * has rows to avoid duplicate-key errors on restart.
     *
     * @since v2026.1.0
     */
     ...
     *
     * <p>Inserts All Employees, Builders, and Admins groups. Skipped when the table already
     * has rows.
     *
     * @since v2026.1.0
     */
    ...
    /**
     * Seeds the {@code permission_catalog} table with the full set of platform permissions.
     *
     * <p>Each permission has a unique key, a display category, a human-readable description,
     * and a risk level ({@code low}, {@code medium}, {@code high}, or {@code critical}).
     ...
     *
     * <p>Each role stores its granted permission keys as a JSON array string. System roles
     * (flagged {@code 1}) cannot be deleted by administrators via the UI. Skipped when the
    ...
    /**
     * Sentinel value that indicates the admin password has not been changed from its default.
     *
     * <p>When {@link AppConfig#getDefaultAdminPassword()} returns this value, a cryptographically
     * random 16-character password is generated instead and printed to the log at WARN level.
     */
    private static final String FIRST_BOOT_SENTINEL = "CHANGE_ME_ON_FIRST_BOOT";

    /**
     * Generates a cryptographically random 16-character password from a mixed-character alphabet.
     *
     * <p>Uses {@link SecureRandom} with an alphabet of upper-case letters, lower-case letters,
    ...
    /**
     * Seeds the {@code users} table with default admin and sample employee accounts.
     *
     * <p>If the configured admin password equals {@link #FIRST_BOOT_SENTINEL} a random
     * password is generated and logged once at WARN level — administrators should copy it
     * from the log on first boot and then update it via the UI. All passwords are BCrypt-hashed
     * with cost factor 12 before storage. Group memberships are inserted with
     * {@code INSERT OR IGNORE} to prevent duplicate-key errors. Skipped when the table already
     * has rows.
     *
     * @since v2026.1.0
     */
