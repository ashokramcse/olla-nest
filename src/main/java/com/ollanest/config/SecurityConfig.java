 ...
 * <p>
 * Spring Security is retained on the classpath for its filter-chain
 * infrastructure and BCrypt password hashing support. However, all of its
 * opinionated default behaviours — form login, HTTP Basic, CSRF token
 * management, default session handling, and the standard security-header set —
 * are explicitly disabled because Olla Nest implements its own cookie-based
 * session authentication via {@link SessionAuthFilter} and its own header
 * policy via {@link SecurityHeadersFilter}.
 *
 * <h3>Design notes</h3>
 * <ul>
 * <li>All HTTP requests are permitted at the Spring Security layer; actual
 * authentication and authorisation are enforced inside the custom filters and
 * the {@code BaseController} helper methods ({@code requireAuth()},
 * {@code requireAdmin()}).</li>
 * <li>CSRF protection is implemented manually via the {@code X-Olla-CSRF}
 * header check in {@code BaseController.requireAuthWithCsrf()}.</li>
 ...
 * <li>v2026.1.4 — no functional changes; retained as part of security audit
 * pass</li>
 * </ul>
 *
 * @author Ashok Ram
...
public class SecurityConfig {
	 ...
	 *
	 * @param sessionAuthFilter     the filter that validates Olla Nest session
	 *                              cookies
	 * @param securityHeadersFilter the filter that sets HTTP security response
	 *                              headers
	 * @since v2026.1.0
	 */
	 ...
	 * <li>Disable the default Spring Security header set
	 * ({@link SecurityHeadersFilter} applies its own)</li>
