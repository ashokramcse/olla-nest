 ...
 * and authorised users before a connection is established.
 *
 * <h3>Why this class exists</h3>
 * <p>
 * Created as part of the CRIT-1 security fix. Without this interceptor, any
 * HTTP client (including unauthenticated ones) could upgrade to a WebSocket
 * connection on {@code /api/terminal} and spawn an arbitrary shell process via
 * {@link com.ollanest.service.TerminalService}. By rejecting the upgrade at
 * handshake time, no session or process setup overhead is incurred for
 ...
 * <li>Admins bypass the explicit {@code workspace:build} right check because
 * the admin role implies all rights.</li>
...
public class WebSocketAuthInterceptor implements HandshakeInterceptor {
	 ...
	 * <li>Returns {@code false} (which produces an HTTP 403 response) if any check
	 * fails.</li>
	 * </ol>
	 *
	 * @param request    the HTTP upgrade request
	 * @param response   the HTTP upgrade response (not modified by this
	 *                   interceptor)
	 * @param wsHandler  the target WebSocket handler (not used in this interceptor)
	 * @param attributes the session attributes map (not populated by this
	 *                   interceptor)
	 * @return {@code true} if the handshake should proceed; {@code false} to reject
