 ...
 * <p>Exposes a single unified endpoint for text-to-image generation that delegates
 * to the configured image provider (OpenAI DALL-E, Stable Diffusion via Ollama, or
 * any other provider supported by {@link ImageGenerationService}). The controller
 * handles auth, input validation, activity logging, and error normalisation so that
 * calling code always receives a consistent JSON envelope.
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>The {@code provider} field in the request body is optional; when absent the
 *       service auto-selects the best available provider.</li>
 ...
 *   <li>v2026.1.4 — added provider auto-selection and base64 response support</li>
 * </ul>
 *
 * @author  Ashok Ram
 * @since   v2026.1.0
...
public class ImageController extends BaseController {
     ...
     *
     * <p>Endpoint: {@code POST /api/images/generate}
     *
     * <p>Processing steps:
     * <ol>
     ...
     * </ol>
     *
     * <p>The response body always contains {@code ok} and {@code provider}. On
     * success it additionally contains either {@code imageUrl} (for DALL-E / remote
     * providers) or {@code base64} (for local providers), but never both.
     *
     * @param  body  JSON request body with:
     *               <ul>
     *                 <li>{@code prompt} (String, required) — the image description</li>
     ...
     * @return       200 OK with {@code {ok: true, provider, imageUrl?|base64?}} on success;
     *               400 if {@code prompt} is blank;
     *               401 if the caller is not authenticated;
     *               500 with {@code {ok: false, error}} on provider error
