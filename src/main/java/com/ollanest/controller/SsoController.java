...
/**
 * SSO authentication endpoints for Google OAuth 2.0, generic OIDC, and SAML 2.0.
 *
 * <h3>Why this class exists</h3>
 * <p>Provides the browser-facing redirect, callback, and assertion consumer endpoints
 * required by the three supported identity-provider protocols. All three flows produce
 * an identical session cookie by calling {@link AuthService#setSession}, so the rest of
 * the application does not need to distinguish SSO sessions from password sessions.
 *
 * <h3>Design notes</h3>
 * <ul>
 *   <li>A one-time CSRF state nonce is stored in the DB before each IdP redirect and
 *       validated at callback time — this prevents CSRF attacks against the OAuth
 *       callback endpoint.</li>
 *   <li>New SSO users are auto-provisioned with role {@code "user"} on first login;
 *       their {@code auth_provider} column records the provider type so the account
 *       settings page can hide fields owned by the IdP.</li>
 *   <li>Admin CRUD endpoints for managing SSO provider records are also grouped here
 *       under {@code /api/auth/sso/admin/providers}.</li>
 *   <li>Client secrets are stored AES-encrypted via {@link CryptoService#encryptKey} —
 *       never in plaintext.</li>
 * </ul>
 *
 ...
 *   <li>v2026.1.4 — added SAML 2.0 ACS endpoint and admin CRUD for provider management</li>
 * </ul>
 *
 * @author  Ashok Ram
 * @since   v2026.1.0
...
public class SsoController {
     ...
     *
     * <p>Endpoint: {@code GET /api/auth/sso/providers}
     *
     * <p>This endpoint is intentionally unauthenticated. Only the provider
     * {@code id}, {@code type}, and {@code name} are included — no client secrets
     * or configuration details are exposed.
     *
     ...
     *
     * <p>Endpoint: {@code GET /api/auth/sso/authorize/{providerId}}
     *
     * <p>A CSRF state nonce is generated and stored in the DB before the redirect.
     * The optional {@code redirectUri} is embedded in the state so the callback
     * can forward the user to the intended destination after successful login.
     * Supports {@code google} and {@code oidc} provider types.
     *
     * @param  providerId   the SSO provider ID (matches a row in {@code sso_providers})
     ...
     *
     * <p>Endpoint: {@code GET /api/auth/sso/callback}
     *
     * <p>Processing steps:
     * <ol>
     *   <li>Validate the {@code state} nonce; redirect to {@code /login?sso_error=invalid_state}
     *       on failure.</li>
     ...
     *   <li>Provision the user via {@link #provisionUser(SsoService.ClaimsResult)}
     *       if this is their first login.</li>
     *   <li>Issue a session cookie and redirect the browser to the intended destination.</li>
     * </ol>
     *
     * @param  code   the authorization code returned by the IdP
     * @param  state  the CSRF nonce echoed back by the IdP
     * @param  error  an OAuth error code from the IdP; triggers redirect to error page if set
     ...
     *
     * <p>Endpoint: {@code POST /api/auth/sso/saml/acs}
     *
     * <p>The SAML response is parsed and verified by
     * {@link SsoService#parseSamlResponse(String)}. On parse failure, the user is
     * redirected to {@code /login?sso_error=saml_parse_failed}. The {@code RelayState}
     * parameter, if present, is reserved for future post-login redirect support.
     *
     * @param  samlResponse  the Base64-encoded {@code SAMLResponse} form parameter
     *                       posted by the IdP
     * @param  relayState    optional opaque relay state from the IdP; currently unused
     * @param  req           the current HTTP request (used for setting the session cookie)
     ...
     *
     * <p>Endpoint: {@code GET /api/auth/sso/admin/providers}
     *
     * <p>Unlike the public {@link #listProviders()} endpoint, this response includes
     ...
     * @return      200 OK with {@code {ok: true, providers: [...]}} on success;
     *              401 if unauthenticated; 403 if the caller is not an admin
     * @since  v2026.1.4
     */
     ...
     *
     * <p>Endpoint: {@code POST /api/auth/sso/admin/providers}
     *
     * <p>The client secret, if provided, is AES-encrypted before storage. The
     * generated provider ID has the form {@code sso-<type>-<base36timestamp>}.
     *
     * @param  body  JSON request body with:
     *               <ul>
     *                 <li>{@code type} (String) — {@code "google"}, {@code "oidc"}, or {@code "saml"}</li>
     *                 <li>{@code name} (String) — display name shown on the login page</li>
     *                 <li>{@code clientId} (String) — OAuth client ID</li>
     *                 <li>{@code clientSecret} (String, optional) — OAuth client secret</li>
     *                 <li>{@code configJson} (String, optional) — provider-specific JSON config</li>
     *               </ul>
     * @param  req   the current HTTP request (must carry an admin session)
     ...
     *
     * <p>Endpoint: {@code PATCH /api/auth/sso/admin/providers/{id}}
     *
     * <p>Only the fields present in the request body are updated. Supported fields:
     * {@code enabled}, {@code name}, {@code configJson}, {@code clientSecret}.
     * An empty {@code clientSecret} string is ignored (does not clear an existing secret).
     *
     ...
     *
     * <p>Endpoint: {@code DELETE /api/auth/sso/admin/providers/{id}}
     *
     * <p>Existing SSO users whose {@code auth_provider} matches this provider will
    ...
    /**
     * Provisions a user account from IdP claims, creating one if it does not yet exist.
     *
     * <p>If a user with the given email already exists, they are returned as-is.
     * Otherwise a new {@code "user"}-role account is created with a {@code NULL}
     * password hash (preventing password-based login) and the provider type recorded
     * in {@code auth_provider}.
     *
     * @param  claims  the verified identity claims extracted from the IdP token or assertion
    ...
    /**
     * Guards admin-only endpoints; returns an error response if the caller is not an admin.
     *
     * @param  req  the current HTTP request
     * @return      {@code null} if the caller is an authenticated admin;
     *              401 response if unauthenticated; 403 response if not an admin
     * @since  v2026.1.0
     */
