 ...
 *
 * <p>GET {@code /api/admin/health} returns JVM memory usage, server uptime,
 * HTTP request/error counters from {@link MonitorService}, and DB-level stats
 * (active users, active models, live sessions, total chats, enabled providers).
 *
 * <p>All fields in the response are named consistently with the frontend
 ...
 *       response used different key names ({@code uptime}, {@code usedMb}) that
 *       did not match what the frontend expected.</li>
 * </ul>
 *
 * @author  Ashok Ram
 * @since   v2026.1.0  — initial Java Spring Boot migration
 * @version v2026.1.0  — bug fix: fixed key name mismatch (uptimeMs, memoryUsedMb, memoryTotalMb)
 */
@RestController
...
public class AdminHealthController extends BaseController {
    ...
    public ResponseEntity<Map<String, Object>> health(HttpServletRequest req) {
        ...
        Integer userCount = db.queryForObject(
                "SELECT COUNT(*) FROM users WHERE active = 1", Integer.class);
        Integer modelCount = db.queryForObject(
                "SELECT COUNT(*) FROM models WHERE status = 'active'", Integer.class);
        Integer sessionCount = db.queryForObject(
                "SELECT COUNT(*) FROM sessions WHERE expires_at > ?",
                Integer.class, Instant.now().toString());
        Integer chatCount = db.queryForObject(
                "SELECT COUNT(*) FROM chat_sessions", Integer.class);
        Integer providerCount = db.queryForObject(
                "SELECT COUNT(*) FROM api_providers WHERE enabled = 1", Integer.class);
