 ...
 *   <li>{@code GET /api/auth/me} — returns authenticated status and user object</li>
 * </ul>
 *
 * <p>Login is protected by IP-based rate limiting (max 10 attempts per 15 minutes).
 * A trusted proxy may be configured to extract the real client IP from the
 * {@code X-Forwarded-For} header.
 *
 * <p>The {@code access_expires_at} column is enforced at login time: if the account
 * has a non-null expiry in the past, login is denied (HIGH-2 security fix).
 *
 * <p><b>Design decisions:</b>
 * <ul>
 *   <li>Rate limit state is stored in the {@code login_attempts} DB table rather
 *       than in-memory so it survives restarts and works correctly with multiple
 *       JVM instances.</li>
 *   <li>The same 401 error message is returned for bad password, unknown email, and
 *       expired account to prevent user enumeration.</li>
 * </ul>
 *
 * @author  Ashok Ram
 * @since   v2026.1.0  — initial Java Spring Boot migration
 * @version v2026.1.0  — security hardening: enforce access_expires_at, session invalidation on role change (HIGH-2)
 */
@RestController
...
public class AuthController extends BaseController {
     ...
     * {@code X-Forwarded-For} header is used to extract the real client IP for
     * rate limiting. If blank, {@code remoteAddr} is used directly.
     */
    ...
    /**
     * Authenticates the user with email and password, issuing a session cookie on success.
     *
     * <p>Processing steps:
     * <ol>
     *   <li>Resolve the client IP (optionally from {@code X-Forwarded-For} via trusted proxy).</li>
     *   <li>Check the {@code login_attempts} table; return 429 if the IP is over the limit.</li>
     ...
     *   <li>On success: clear rate-limit record, set session cookie, write audit event.</li>
     *   <li>On failure: increment rate-limit counter, return 401.</li>
     * </ol>
     *
     * @param  body  request body with {@code email} (String) and {@code password} (String)
     * @param  req   the current HTTP request (for IP extraction)
     * @param  res   the HTTP response (session cookie is set here on success)
     * @return       200 OK with {@code {ok, user, redirectTo}} on success;
     *               400 for missing fields; 401 for bad credentials; 429 for rate limit
     * @since   v2026.1.0  — initial Java Spring Boot migration
    ...
    public ResponseEntity<Map<String, Object>> login(@RequestBody Map<String, Object> body,
        ...
        List<Map<String, Object>> rows = db.queryForList(
                "SELECT id, name, email, role, rights, department_id, active, employee_id, "
                        ...
                        + "OR access_expires_at > datetime('now'))",
                email);
     ...
     *
     * <p>Requires the {@code X-Requested-With} CSRF header to prevent CSRF-driven
     ...
     *
     * <p>This is an unauthenticated-safe endpoint — it returns
