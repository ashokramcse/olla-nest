 ...
 *
 * <p>All endpoints require a valid session cookie (checked via
 * {@link BaseController#requireAuthWithCsrf}). Endpoints provided:
 * <ul>
 *   <li>{@code POST /api/account/password} — change own password (BCrypt cost 12)</li>
 *   <li>{@code PATCH /api/account/profile} — update own mutable profile fields</li>
 *   <li>{@code GET /api/account/profile} — retrieve own profile including department name</li>
 *   <li>{@code GET /api/account/usage} — retrieve daily and monthly token usage vs limits</li>
 * </ul>
 *
 * <p><b>Design decisions:</b>
 * <ul>
 *   <li>Enterprise users (non-local auth provider) may not update designation, team,
 *       or branch — those fields are owned by the identity provider.</li>
 *   <li>Profile field updates use an allowlist to prevent mass-assignment attacks.</li>
 * </ul>
 *
 * @author  Ashok Ram
...
public class AccountController extends BaseController {
     ...
     *
     * <p>Requires the current password to be supplied for verification. The new
    ...
    public ResponseEntity<Map<String, Object>> changePassword(@RequestBody Map<String, Object> body,
        ...
        db.update("UPDATE users SET password_hash = ? WHERE id = ?",
                BCrypt.hashpw(newPassword, BCrypt.gensalt(12)), user.id);
     ...
     *
     * <p>Fields always allowed: {@code name}, {@code phone}, {@code avatar_initials}.
    ...
    public ResponseEntity<Map<String, Object>> updateProfile(@RequestBody Map<String, Object> body,
        ...
        User user = getUser(req);
        List<Map<String, Object>> rows = db.queryForList(
                "SELECT auth_provider FROM users WHERE id = ?", user.id);
        if (rows.isEmpty()) return ResponseEntity.status(404).body(Map.of("error", "User not found"));
    ...
    /**
     * Returns the authenticated user's profile, including the resolved department name.
     *
     * @param  req  the current HTTP request (for auth and CSRF checks)
     * @return      200 OK with {@code {user: User, departmentName: String}},
     *              or 404 if the user record no longer exists
     * @since   v2026.1.0  — initial Java Spring Boot migration
     */
    @GetMapping("/profile")
    public ResponseEntity<Map<String, Object>> getProfile(HttpServletRequest req) {
        ...
        if (fresh.departmentId != null) {
            List<Map<String, Object>> depts = db.queryForList(
                    "SELECT name FROM departments WHERE id = ?", fresh.departmentId);
    ...
    /**
     * Returns the authenticated user's token consumption for today and the current month.
     *
     * <p>Counts tokens from {@code chat_messages} rows where {@code role = 'assistant'}
     * and the message belongs to a session owned by the current user.
     *
     * @param  req  the current HTTP request (for auth and CSRF checks)
     * @return      200 OK with {@code {tokensUsedToday, dailyTokenLimit, tokensUsedMonth, monthlyTokenLimit}}
     * @since   v2026.1.0  — initial Java Spring Boot migration
     */
    @GetMapping("/usage")
    public ResponseEntity<Map<String, Object>> getUsage(HttpServletRequest req) {
        ...
        String todayStart = Instant.now().atZone(ZoneId.of("UTC")).toLocalDate()
                .atStartOfDay(ZoneId.of("UTC")).toInstant().toString();
        // Current month start UTC
        String monthStartStr = Instant.now().atZone(ZoneId.of("UTC")).toLocalDate()
                .withDayOfMonth(1).atStartOfDay(ZoneId.of("UTC")).toInstant().toString();
     ...
     *
     * <p>Example: {@code "avatar_initials"} → {@code "avatarInitials"}.
     *
