 ...
 * <p>
 * Helps users pick and provision local models that fit their machine. It detects
 * the host hardware, serves a recommended model catalog, and (for admins) drives
 * model downloads from Hugging Face with live progress streamed over SSE. The
 * heavy lifting — hardware probing, catalog assembly, and download orchestration —
 * lives in {@link CookbookService}.
 *
 * <h3>Design notes</h3>
 * <ul>
 ...
 * <li>{@link #download} returns an {@link SseEmitter} with a two-hour timeout so
 * the client can follow long-running downloads in real time.</li>
 * </ul>
 *
...
public class CookbookController extends BaseController {
    ...
    /**
     * Starts a model download from Hugging Face, streaming progress over SSE
     * (admin only).
     *
     * @param req  the HTTP request, used to resolve the authenticated user
