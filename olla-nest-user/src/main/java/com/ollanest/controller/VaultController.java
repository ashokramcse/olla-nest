 ...
 * <p>
 * Lets the application retrieve credentials from a Bitwarden-compatible vault
 * via the {@code bw} CLI. Configuration, unlocking with the master password,
 * and item retrieval are sensitive operations, so they are restricted to
 * administrators; only the read-only lock status is available to any
 * authenticated user. The CLI interaction and session handling are delegated to
 * {@link VaultService}.
 *
 * <h3>Design notes</h3>
 * <ul>
...
public class VaultController extends BaseController {
	...
	public ResponseEntity<?> unlock(HttpServletRequest req, @RequestBody Map<String, Object> body) {
		...
		if (password == null || password.isBlank())
			return badRequest("master_password is required");
