...
- **Root cause:** `ContactsService.delete` / `TaskSchedulerService.delete` ran `db.update("DELETE … WHERE id=? AND owner=?")` and ignored the affected-row count; the controller always returned `ok`.
- **Fix:** both now check the affected-row count and throw `NoSuchElementException` (→ 404 via `GlobalExceptionHandler`) when 0 rows matched — consistent with `SkillsService.deleteSkill`.
- **Verification (live, post-fix):** delete of a non-existent contact/task → **404**; delete of an owned contact → 200. Regression tests `throwsWhenNothingDeleted` added to both service tests (+ existing delete tests now stub the row count). Full `mvn test` BUILD SUCCESS.
