 ...
 *
 * <p>
 * Verifies: authenticated request sets "authenticatedUser" attribute,
 * unauthenticated request does not set attribute but still calls chain, filter
...
class SessionAuthFilterTest {
	...
	void doesNotSetAttributeForUnauthenticatedRequest() throws Exception {
		...
		// No attribute must be set — controllers check for null to detect
		// unauthenticated state
		verify(request, never()).setAttribute(anyString(), any());
		// Chain must still continue — this filter never blocks requests (controllers do
		// that)
	...
	void chainAlwaysContinuesEvenIfAuthServiceThrows() throws Exception {
		// AuthService swallows its own exceptions; getSessionUser returns null on
		// error.
	...
	void setsExactUserInstance() throws Exception {
		...
		// Must store the exact same reference — not a copy — so identity checks in
		// controllers work
