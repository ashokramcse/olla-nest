 ...
 *
 * <p>Validates: parameterized query enforcement, enum guards, table-name
 * allow-list, compound-UPDATE atomicity, input-length limits, LIMIT guards,
...
class SqlSafetyTest {
    ...
    class ModelStatusEnumGuard {
        ...
        /**
         * Validates that the ALLOWED_STATUSES set in AdminModelsController
         * covers exactly the lifecycle values used by the router and Ollama sync.
         * Tests act as a contract: adding a new status MUST also update this test.
         */
        ...
        @ValueSource(strings = {
            "'; DROP TABLE models; --",   // injection attempt
    ...
    class ParameterizedQueryEnforcement {
        ...
        void getSettingIsParameterized() {
            ...
            assertThat(sqlCaptor.getValue())
                    .contains("?")
                    .doesNotContain("myKey"); // key must NOT appear literally in SQL
        }
        ...
        void removeSessionIsParameterized() {
            ...
            assertThat(sqlCaptor.getValue())
                    .contains("?")
                    .doesNotContain(token); // token must NOT appear literally in SQL
        }
        ...
        void getSessionUserIsParameterized() {
            ...
            verify(db).queryForList(
                    argThat(sql -> sql.contains("?") && !sql.contains("a".repeat(64))),
                    eq("a".repeat(64)));
        }
    }
    ...
    class LimitProtection {
        ...
        /**
         * Verifies the LIMIT constants used in production queries are bounded.
         * These tests document the agreed upper bounds and will fail if someone
         * removes or increases a LIMIT beyond safe thresholds.
         */
        @Test
    ...
    class CompoundUpdateAtomicity {
        ...
        void compoundUpdateSqlIsValid() {
            ...
            assertThat(sql)
                    .startsWith("UPDATE models SET")
                    .contains("status = ?")
                    .contains("governance_tier = ?")
                    .contains("max_context_size = ?")
                    .endsWith("WHERE id = ?")
                    .doesNotContain("'")  // no literal values — all parameterized
    ...
    class InputLengthValidation {
        ...
        private static final Map<String, Integer> FIELD_MAX_LENGTHS = Map.of(
                "name",           150,
                "phone",           30,
                "avatarInitials",   4,
                "designation",    100,
    ...
    class VacuumIntoSafety {
        ...
        void singleQuotesEscaped() {
            ...
            // After escaping, no lone (unescaped) single quote can terminate the SQL string.
            ...
            // All single quotes are now doubled — no lone ' exists to terminate the SQL string
            assertThat(escaped).contains("''"); // quotes were doubled
            // Verify there is no lone single-quote followed by semicolon (the injection sequence)
    ...
    class DestructiveOperationGuards {
        ...
        void noUnrestrictedMutations() {
            ...
            // The actual production SQL is tested in SchemaIntegrationTest and mock tests above.
            // Here we document the invariant: WHERE must accompany every mutation.
            String[] unsafePatterns = {
                "DELETE FROM users",         // must always have WHERE
