 ...
 *
 * <p>
 * Covers: {@code cleanModelOutput()} — null safe, think-tag stripping, unclosed
 * think-tag with content markers; {@code extractArtifacts()} — fenced code
 * blocks, language detection, bare HTML fallback, no artifacts for plain text;
 * {@code normalizePermissionMode()} — known modes, unknown falls back to
 * "default".
 *
 * <p>
 * No FS access, no Spring context. These are all pure computation paths.
 *
 * @author Ashok Ram
