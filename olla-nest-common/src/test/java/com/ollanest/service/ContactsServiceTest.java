 ...
 *
 * <p>
 * Covers: {@code create()} — DB INSERT, ID prefix, owner; {@code update()} — DB
 * UPDATE; {@code delete()} — scoped DELETE; {@code getById()} — null when not
 * found; {@code list()} — owner and limit parameters.
 *
 * <p>
 * All DB and {@link ObjectMapper} interactions are Mockito-stubbed — no Spring
 * context, no real DB.
 *
 * @author Ashok Ram
...
class ContactsServiceTest {
	...
	class Create {
		...
		void displayNameStored() {
			...
			verify(db).update(contains("INSERT INTO contacts"), cap.capture());
			// args[0] = id, args[1] = owner, args[2] = display_name — verify correct
			// position
	...
	class GetById {
		...
		void returnsMappedRow() {
			var row = Map.<String, Object>of("id", "cnt-1", "owner", OWNER, "display_name", "Alice", "email_json", "[]",
					"phone_json", "[]", "address_json", "[]");
			// Stub: contact found
