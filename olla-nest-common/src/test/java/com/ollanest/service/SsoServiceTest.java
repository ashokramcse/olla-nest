 ...
 *
 * <p>
 * Covers state management, provider listing, and Google auth URL construction.
 *
 * @author Ashok Ram
 * @since v2026.2.1
 * @version v2026.2.1
...
class SsoServiceTest {
	...
	class CreateState {
		...
		void stateIs64Hex() {
			// SECURITY: 64 hex chars = 256 bits of entropy — prevents CSRF via state
			// forgery
	...
	class ListEnabledProviders {
		...
		void queriesForEnabled() {
			...
			when(db.queryForList(contains("WHERE enabled = 1")))
					.thenReturn(List.of(Map.of("id", "sso-google", "type", "google", "name", "Google")));
			List<Map<String, Object>> results = ssoService.listEnabledProviders();
			// Only enabled providers must be returned — disabled ones must be hidden from
			// login UI
	...
	class BuildGoogleAuthUrl {
		...
		void containsHdParam() {
			// hd= restricts login to a specific Google Workspace domain — used for
			// enterprise SSO
		...
		void noHdParamWhenNull() {
			// Without a hosted domain, any Google account can authenticate — hd= must be
			// absent
			String url = ssoService.buildGoogleAuthUrl("client-123", "state-abc", null);
