 ...
 *
 * <p>
 * Covers CRUD operations for MCP server management.
 *
 * @author Ashok Ram
 * @since v2026.2.1
 * @version v2026.2.1
...
class McpServerServiceTest {
	...
	private Map<String, Object> serverRow(String id, String name) {
		// LinkedHashMap (not Map.of) because the "url" value is null, which Map.of
		// rejects.
	...
	class Create {
		...
		void insertsRow() throws Exception {
			// Stub: mapper serializes args/env lists to JSON; DB read-back returns created
			// row
			when(mapper.writeValueAsString(any())).thenReturn("[]");
			when(db.queryForList(anyString(), anyString())).thenReturn(List.of(serverRow("mcp-abc", "My Server")));
		...
		void idPrefix() throws Exception {
			...
			when(mapper.writeValueAsString(any())).thenReturn("[]");
			when(db.queryForList(anyString(), anyString())).thenReturn(List.of(serverRow("mcp-xyz", "Test")));
		...
		void nameStored() throws Exception {
			...
			when(mapper.writeValueAsString(any())).thenReturn("[]");
			when(db.queryForList(anyString(), anyString())).thenReturn(List.of(serverRow("mcp-1", "My Server")));
	...
	class GetById {
		...
		void returnsMappedRow() throws Exception {
			...
			when(db.queryForList(anyString(), eq("mcp-1"))).thenReturn(List.of(serverRow("mcp-1", "Server1")));
	...
	class Delete {
		...
		void deletesById() {
			...
			// Parameterized DELETE — ID must appear as a bound parameter, never in the SQL
			// string
	...
	class SetDisabledTools {
		...
		void updatesWithJson() throws Exception {
			...
			verify(db).update(contains("UPDATE mcp_servers SET disabled_tools_json=?"), anyString(), anyString(),
					eq("mcp-1"));
		}
	}
}
