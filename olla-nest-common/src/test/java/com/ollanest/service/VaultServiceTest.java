 ...
 *
 * <p>
 * Covers: {@code getConfig()} — returns {@code enabled=false} when no row
 * found; {@code getConfig()} — never exposes {@code session_enc};
 * {@code saveConfig()} — calls UPDATE first; INSERT called when no row updated
 * (upsert pattern). Methods that spawn real processes ({@code unlock()},
 * {@code lock()}, {@code getItem()}) are not tested here.
 *
 * <p>
 * All DB and {@link CryptoService} interactions are Mockito-stubbed.
 *
 * @author Ashok Ram
...
class VaultServiceTest {
	...
	class GetConfig {
		...
		void sessionEncNeverExposed() {
			...
			var row = Map.<String, Object>of("id", "singleton", "bw_path", "bw", "server_url",
					"https://vaultwarden.example.com", "enabled", 1, "session_enc",
					"super-secret-encrypted-session-key");
		...
		void returnsConfigFields() {
			...
			var row = Map.<String, Object>of("id", "singleton", "bw_path", "/usr/bin/bw", "server_url",
					"https://vault.example.com", "enabled", 1);
			when(db.queryForList(contains("FROM vault_config"))).thenReturn(List.of(row));
			var result = svc.getConfig();
			// Non-secret fields (bw_path, server_url) must be returned so the UI can
			// display them
	...
	class SaveConfig {
		...
		void noInsertWhenUpdateSucceeds() {
			// Stub: UPDATE row count = 1 → no INSERT should fire (avoid duplicate singleton
			// row)
			when(db.update(contains("UPDATE vault_config"), any(), any(), any())).thenReturn(1);
