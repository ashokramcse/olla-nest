 ...
 *
 * <p>
 * Covers token estimation, compaction threshold detection, and compact()
 * message reduction.
 *
 * @author Ashok Ram
 * @since v2026.2.1
...
class ContextCompactorServiceTest {
	...
	class NeedsCompaction {
		...
		void falseWhenFit() {
			...
			List<Map<String, Object>> msgs = List.of(Map.of("role", "user", "content", "hi") // ~1 token
			);
		...
		void trueWhenOverThreshold() {
			...
			List<Map<String, Object>> msgs = List.of(Map.of("role", "user", "content", bigContent));
		...
		void usesDefaultWhenZero() {
			List<Map<String, Object>> msgs = List.of(Map.of("role", "user", "content", "hello"));
	...
	class Compact {
		...
		void reduceMessageCount() {
			...
			List<Map<String, Object>> compacted = contextCompactorService.compact(msgs, 8192, "http://localhost:11434",
					"llama3.2");
		...
		void lastMessagePreserved() {
			...
			List<Map<String, Object>> compacted = contextCompactorService.compact(msgs, 8192, "http://localhost:11434",
					"llama3.2");
			// Most recent user message must always be in the compacted result — it's the
			// current query
