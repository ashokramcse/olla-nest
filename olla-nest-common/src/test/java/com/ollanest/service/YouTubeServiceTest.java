 ...
 *
 * <p>
 * Covers: {@code getTranscript()} — returns cached transcript when DB hit;
 * returns null for unextractable URLs; returns null for null/blank input; all
 * common YouTube URL patterns extract the correct video ID (tested via DB cache
 * hit path).
 *
 * <p>
 * The private {@code extractVideoId()} is tested indirectly via
 * {@code getTranscript()}.
 *
 * @author Ashok Ram
...
class YouTubeServiceTest {
	...
	class NullAndInvalid {
		...
		@ParameterizedTest
		@ValueSource(strings = { "https://example.com", "not-a-url", "ftp://youtube.com", "dQw4w9WgXc" // 10 chars — too
																										// short for
																										// direct ID
		})
