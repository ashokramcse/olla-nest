 ...
 * rather than by offset so the operation is safe against concurrent
 * inserts.</li>
 * </ul>
 *
...
public class SessionEnhancementService {
	...
	/**
	 * Triggers LLM-based topic detection for a session in a background virtual
	 * thread. Detected topics are stored as the session title if no meaningful
	 * title exists yet.
	 *
	 * @param sessionId the chat session ID to analyse
	 * @param userId    the owner user ID (used for ownership checks inside the
	 *                  async task)
	 * @since v2026.2.1
	 */
	...
	private void analyzeTopics(String sessionId, String userId) throws Exception {
		...
		msgs.reversed().forEach(m -> transcript.append(m.get("role")).append(": ")
		...
		String content = mapper.readTree(HTTP.send(req, HttpResponse.BodyHandlers.ofString()).body()).path("message")
				.path("content").asText("[]");
	...
	/**
	 * Forks a conversation at a given message index, creating a new session with
	 * all messages up to and including that index. The original session is not
	 * modified.
	 *
	 * @param sessionId    the session to fork
	 * @param userId       the owner user ID (must match the session's user_id)
	 * @param messageIndex zero-based index of the last message to include in the
	 *                     fork
	 * @return a map with {@code forked_session_id}, {@code message_count}, and
	 *         {@code original_session_id}
	 * @throws NoSuchElementException if the session is not found or not owned by
	 *                                the user
	 * @since v2026.2.1
	 */
	public Map<String, Object> forkSession(String sessionId, String userId, int messageIndex) {
		...
		List<Map<String, Object>> messages = db.queryForList(
				"SELECT * FROM chat_messages WHERE session_id=? ORDER BY created_at ASC LIMIT ?", sessionId,
				messageIndex + 1);
		...
		db.update("INSERT INTO chat_sessions (id, user_id, title, created_at, updated_at) VALUES (?,?,?,?,?)", newId,
				userId, "Fork: " + origTitle, now, now);

		// Copy messages
		for (Map<String, Object> msg : messages) {
			String newMsgId = "msg-fork-" + Long.toString(System.currentTimeMillis(), 36)
	 ...
	 * @param fromMessageId the ID of the first message to delete; all messages at
	 *                      or after its {@code created_at} timestamp are removed
	...
	public void truncateHistory(String sessionId, String userId, String fromMessageId) {
		...
		var pivot = db.queryForList("SELECT created_at FROM chat_messages WHERE id=? AND session_id=?", fromMessageId,
				sessionId);
		if (pivot.isEmpty())
			throw new NoSuchElementException("Message not found: " + fromMessageId);

		String pivotDate = (String) pivot.get(0).get("created_at");
		int deleted = db.update("DELETE FROM chat_messages WHERE session_id=? AND created_at >= ?", sessionId,
				pivotDate);
