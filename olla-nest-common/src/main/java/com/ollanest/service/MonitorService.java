 ...
 * <p>
 * Provides lightweight, zero-dependency request and error counters together
 * with JVM memory statistics for the {@code GET /api/health} endpoint. This
 * avoids pulling in a full metrics library (Micrometer/Prometheus) for a single
 * internal health check while still giving operators the data they need to
 * confirm the process is alive and to catch error spikes.
 *
 * <h3>Design notes</h3>
 * <ul>
 * <li>All counters use {@link AtomicLong} for thread safety without
 * synchronization overhead.</li>
 * <li>Uptime is calculated from {@code System.currentTimeMillis()} captured at
 * construction time rather than from
 * {@link ManagementFactory#getRuntimeMXBean()} so that the reported uptime
 * reflects application-context startup, not JVM startup.</li>
 * <li>The import of {@link ManagementFactory} is retained for potential future
 * use; it is not invoked at runtime.</li>
 * </ul>
 *
 * <h3>Version history</h3>
 * <ul>
 * <li>v2026.1.0 — initial migration; replaces Node.js in-memory counter
 * object</li>
...
public class MonitorService {
	...
	/**
	 * Epoch millisecond at which this service was instantiated (proxy for app start
	 * time).
	 */
	 ...
	 *
	 * <p>
	 * Should be called once per inbound API request, typically from a servlet
	 ...
	 *
	 * <p>
	 * Should be called whenever a request results in a 4xx or 5xx response.
	 *
	 * @since v2026.1.0
	 */
