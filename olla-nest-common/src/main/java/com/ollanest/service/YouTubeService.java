 ...
 * <h3>Design notes</h3>
 * <ul>
 * <li>The caption track URL is extracted from the
 * {@code ytInitialPlayerResponse} JSON embedded in the watch page HTML using a
 * regex; this is the same approach used by popular transcript-extraction
 * libraries.</li>
 * <li>The raw XML is parsed with Jsoup's XML parser, which handles HTML entity
 * escaping in caption text.</li>
 * <li>All formats of YouTube URL are supported via the
 * {@link #VIDEO_ID_PATTERN} regex: {@code watch?v=}, {@code youtu.be/},
 * {@code embed/}, {@code shorts/}.</li>
 * </ul>
 *
...
public class YouTubeService {
	...
	/**
	 * Regex that extracts the 11-character video ID from all common YouTube URL
	 * formats.
	 */
	private static final Pattern VIDEO_ID_PATTERN = Pattern
			.compile("(?:v=|youtu\\.be/|embed/|v/|shorts/)([a-zA-Z0-9_-]{11})");
	...
	/**
	 * Extracts the plain-text transcript for a YouTube video, using a 30-day DB
	 * cache.
	 *
	...
	public String getTranscript(String urlOrId) {
		...
		if (transcript != null) {
			...
			try {
				db.update(
						"INSERT OR REPLACE INTO youtube_transcripts (video_id, title, transcript, cached_at) VALUES (?,?,?,?)",
	...
	private String fetchTranscript(String videoId) {
		try {
			...
			HttpRequest req = HttpRequest.newBuilder().uri(URI.create(pageUrl))
			...
			HttpRequest capReq = HttpRequest.newBuilder().uri(URI.create(captionUrl)).timeout(Duration.ofSeconds(15))
					.GET().build();
	...
	private String fetchTitle(String videoId) {
		try {
			...
			HttpRequest req = HttpRequest.newBuilder().uri(URI.create(url)).header("User-Agent", "Mozilla/5.0")
	...
	/**
	 * Extracts the 11-character YouTube video ID from a URL or returns the input if
	 * it is already a bare video ID.
	 *
	 * @param input a YouTube URL (watch, youtu.be, embed, shorts) or a bare video
	 *              ID
