 ...
 * admin rights, validate the CSRF header, and produce consistent error
 * responses. Placing these helpers in a single base class keeps each concrete
 * controller focused on its own domain and ensures security checks are applied
 * consistently across the application.
 *
 * <h3>Design notes</h3>
 * <ul>
 * <li>Two auth patterns are supported. The <em>legacy pattern</em>
 * ({@link #guardAuth}, {@link #guardAdmin}) returns a {@link ResponseEntity}
 * that callers check against {@code null}. The <em>modern pattern</em>
 * ({@link #requireAuth}, {@link #requireAdminUser}) throws
 * {@link AuthException} or {@link ForbiddenException} which are mapped to HTTP
 * 401/403 by the global exception handler. New controllers should prefer the
 * modern pattern.</li>
...
public abstract class BaseController {
	 ...
	 * {@link com.ollanest.filter.SessionAuthFilter}, or {@code null} if
	 * unauthenticated.
	 *
	...
	/**
	 * Returns the authenticated user, or throws {@link AuthException} (mapped to
	 * HTTP 401) if not logged in. Use in new-style controllers.
	 *
	 * @param req the current HTTP request
	...
	/**
	 * Returns the authenticated admin user, or throws {@link AuthException} (HTTP
	 * 401) if unauthenticated or {@link ForbiddenException} (HTTP 403) if not an
	 * admin.
	 *
	 * @param req the current HTTP request
	...
	/**
	 * Legacy auth guard — returns a 401 {@link ResponseEntity} if not
	 * authenticated, or {@code null} if the request is authenticated. Used by
	...
	/**
	 * Legacy admin guard — returns a 401 or 403 {@link ResponseEntity} if not
	 * admin, or {@code null} if the request is from an authenticated admin.
	 *
	 * @param req the current HTTP request
	...
	/**
	 * Strips HTML tags and escapes special characters to prevent stored XSS.
	...
	/**
	 * Legacy alias so existing code that calls {@code requireAdmin(req)} still
	 * compiles. Delegates to {@link #guardAdmin}.
	 *
	 * @param req the current HTTP request
	 * @return a 401/403 response entity, or {@code null} if the caller is an admin
	 * @since v2026.1.0
