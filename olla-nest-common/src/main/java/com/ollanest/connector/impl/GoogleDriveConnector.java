 ...
 * documents.
 *
 * <h3>Credential format</h3> <pre>{@code { "accessToken": "ya29..." // OAuth
 * 2.0 bearer token; must have drive.readonly scope. // Token refresh is handled
 * externally (SSO or a separate refresh flow). } }</pre>
 *
 * <h3>Design notes</h3> <ul> <li>Google Docs are never stored natively on
 * Drive; they are exported on-the-fly via the {@code
 * /export?mimeType=text/plain} endpoint.</li> <li>PDF files are downloaded as
 * raw bytes and converted to plain text using Apache PDFBox.</li>
 ...
 * sync.</li> <li>The list query is capped at 50 files ordered by {@code
 * modifiedTime desc}; pagination is not yet implemented.</li> </ul>
 *
