...
/**
 * Spring {@code @Component} that pulls Asana tasks into the Olla-Nest knowledge
 * index.
 *
 * <h3>Why this class exists</h3> Teams that live in Asana store a wealth of
 * project context — task descriptions, acceptance criteria, and status notes —
 * that should be searchable via the AI layer. This connector enumerates every
 * workspace the token has access to, then pages through tasks in each workspace
 * and ingests them as Markdown documents so they appear in search results
 ...
 * </pre>
 *
 * <h3>Configuration format</h3> No additional configuration is required. The
 * connector automatically discovers all workspaces visible to the supplied
 * token.
 *
 * <h3>Design notes</h3>
 * <ul>
 * <li>Workspace discovery uses {@code GET /workspaces} so no workspace IDs need
 * to be hard-coded in the connector configuration.</li>
 * <li>Tasks are fetched with {@code opt_fields=gid,name,notes,permalink_url} to
 * minimise response payload size; the {@code notes} field is used as the
 * document body.</li>
 * <li>Up to 100 tasks are retrieved per workspace in a single request
 * ({@code limit=100}); pagination is not yet implemented.</li>
 * <li>Each task is keyed by its {@code gid} so repeat syncs are
 * idempotent.</li>
 * </ul>
 *
...
public class AsanaConnector extends BaseConnector {
	 ...
	 * a persisted connector row whose {@code type} column equals {@code "asana"}.
	 *
	...
	/**
	 * Synchronises all tasks from all workspaces visible to the supplied token with
	 * the Olla-Nest document store.
	 *
	 * <p>
	 * The method performs the following steps:
	 * <ol>
	 * <li>Lists all workspaces accessible by the token via
	 * {@code GET /workspaces}.</li>
	 * <li>For each workspace, fetches up to 100 tasks with name, notes, and
	 * permalink.</li>
	 * <li>Ingests each task as a Markdown document keyed by the task's
	 * {@code gid}.</li>
	 * </ol>
	 *
	...
	public SyncResult sync(Map<String, Object> config, String credentials) {
		...
		try {
			...
			for (JsonNode w : ws.path("data")) {
				...
				JsonNode tasks = httpGet(
						BASE + "/tasks?workspace=" + wsId + "&opt_fields=gid,name,notes,permalink_url&limit=100", auth);
