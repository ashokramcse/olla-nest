 ...
 *
 * <p>Java's {@link Map#getOrDefault(Object, Object)} only substitutes the default
 * when a key is <em>absent</em>. A JSON request body like {@code {"title": null}}
 * deserialises to a present key with a {@code null} value, so {@code getOrDefault}
 * returns {@code null} — which then reaches a {@code NOT NULL} column and surfaces a
 * raw {@code SQLITE_CONSTRAINT_NOTNULL} as an HTTP 500 instead of being handled.
 *
 * <p>{@link #orDefault(Object, Object)} treats both the absent and the explicit-null
 * cases the same way, returning the fallback whenever the value is {@code null}.
 * Use it for every column that is {@code NOT NULL} in the schema (BUG-019).
 *
 * @author Ashok Ram
 * @since v2026.2.2
 * @version v2026.2.2
