...
public class AdminTeamsController extends BaseController {
	 ...
	 *
	 * <p>
	 * Member counts are derived from the {@code user_groups} table.
	 *
	...
	public ResponseEntity<Map<String, Object>> listTeams(HttpServletRequest req) {
		...
		for (Map<String, Object> t : teams) {
			Integer mc = db.queryForObject("SELECT COUNT(*) FROM user_groups WHERE group_id = ?", Integer.class,
					t.get("id"));
	 ...
	 *
	 * <p>
	 * Returns 400 if {@code name} is blank or already exists.
	 *
	 ...
	 *
	 * <p>
	 * Only the {@code name} field is updated; other fields are ignored. Returns 404
	...
	public ResponseEntity<Map<String, Object>> deleteTeam(@PathVariable String id, HttpServletRequest req) {
		...
		chatService.appendAudit(getUser(req).name, "admin.team.delete", "Deleted team " + rows.get(0).get("name"),
				null);
		return ResponseEntity.ok(Map.of("ok", true));
	}
