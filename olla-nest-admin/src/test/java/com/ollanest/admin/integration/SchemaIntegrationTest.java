 ...
 *
 * <p>Uses {@code @SpringBootTest} with the {@code test} profile so Flyway runs
 * all migrations (V1–V6) against a fresh in-memory SQLite instance. The
 ...
 */
@SpringBootTest(classes = OllaNestAdminApplication.class,
...
class SchemaIntegrationTest {
    ...
    @MockitoBean DatabaseService databaseService;
    ...
    class CoreTablesExist {
        ...
        void tableExists(String tableName) {
            ...
            assertThat(count)
                    .as("Table '%s' must exist after Flyway migration", tableName)
                    .isEqualTo(1);
        }
    }
    ...
    class V6PerformanceIndexes {
        ...
        @ValueSource(strings = {
            "idx_models_provider",
            "idx_models_status",
            "idx_models_provider_status",
            "idx_users_email",
            "idx_users_role",
            "idx_users_active",
            "idx_users_email_active",
            "idx_chat_sessions_updated_at",
            "idx_chat_sessions_user_active_updated",
            "idx_audit_events_actor",
            "idx_audit_events_action",
            "idx_connector_sync_log_connector",
            "idx_connector_sync_log_started_at",
            "idx_user_overrides_user",
            "idx_user_overrides_expires",
            "idx_access_grants_subject",
        ...
        void v6IndexExists(String indexName) {
            ...
            assertThat(count)
                    .as("V6 index '%s' must exist after migration", indexName)
                    .isEqualTo(1);
        }
    }
    ...
    class V1BaselineIndexes {
        ...
        @ValueSource(strings = {
            ...
            "idx_chat_messages_user_date",
            "idx_chat_messages_created_at",
            "idx_chat_sessions_user_active",
            "idx_audit_events_created_at",
            "idx_router_traces_created_at",
        ...
        void v1IndexExists(String indexName) {
            Integer count = db.queryForObject(
                    ...
                    Integer.class, indexName);
            assertThat(count)
                    .as("V1 index '%s' must exist", indexName)
                    .isEqualTo(1);
        }
    }
    ...
    class FlywayMigrationHistory {
        ...
        void sixMigrationsApplied() {
            ...
            Integer count = db.queryForObject(
                    ...
                    Integer.class);
            assertThat(count)
                    .as("All 6 Flyway migrations must be recorded as successful")
        ...
        void noFailedMigrations() {
            ...
            assertThat(failCount)
                    .as("No failed Flyway migrations must exist")
                    .isZero();
        }
        ...
        void v6MigrationPresent() {
            Integer count = db.queryForObject(
                    ...
                    Integer.class);
            assertThat(count)
                    .as("V6 migration must be recorded in flyway_schema_history")
                    .isEqualTo(1);
        }
    }
    ...
    class SchemaConstraints {
        ...
        void usersHasAuthColumns() {
            ...
            assertThat(colNames).contains(
                    ...
                    "department_id", "auth_provider", "active"
            );
        }
        ...
        void chatSessionsHasRequiredColumns() {
            ...
            List<String> colNames = cols.stream()
                    ...
                    .toList();
            assertThat(colNames).contains(
                    "id", "user_id", "title", "is_active", "pinned",
                    "archived", "unread", "created_at", "updated_at"
            );
        }
        ...
        void chatMessagesHasLatencyColumn() {
            ...
            List<String> colNames = cols.stream()
                    ...
                    .toList();
            assertThat(colNames).contains(
                    "id", "session_id", "role", "content", "model_id",
        ...
        void modelsHasGovernanceColumns() {
            ...
            List<String> colNames = cols.stream()
                    ...
                    .toList();
            assertThat(colNames).contains(
                    "id", "name", "provider", "status", "governance_tier",
                    "gpu_required", "sensitive_allowed", "max_context_size"
            );
        }
    ...
    class ForeignKeyEnforcement {
        ...
        void foreignKeysEnabled() {
            ...
            assertThat(fkEnabled)
                    .as("SQLite driver must support PRAGMA foreign_keys=ON")
                    .isEqualTo(1);
        }
    }
    ...
    class SettingsTableFunctionality {
        ...
        void settingsInsertAndSelect() {
            db.update("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)",
                    "schema_test_key", "schema_test_value");
            List<Map<String, Object>> rows = db.queryForList(
                    "SELECT value FROM settings WHERE key = ?", "schema_test_key");
        ...
        void settingsInsertOrReplaceIdempotent() {
            db.update("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)",
                    "schema_idempotent_key", "first_value");
            db.update("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)",
                    "schema_idempotent_key", "second_value");
            List<Map<String, Object>> rows = db.queryForList(
                    "SELECT value FROM settings WHERE key = ?", "schema_idempotent_key");
            assertThat(rows).hasSize(1);
        ...
        void schemaObjectCountSane() {
            ...
            assertThat(tableCount)
                    .as("Schema must have at least 28 production tables")
